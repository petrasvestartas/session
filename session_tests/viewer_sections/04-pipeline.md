...
In Chapter 2 the GPU only *cleared* the screen. Now we make it *draw*. We add the
smallest possible thing a GPU can draw: one triangle whose 3 corners are written
**inside the shader** — no data buffers yet. Buffers come in a later chapter; this
chapter proves the drawing machinery works.
...
> The trick that makes this the *simplest* triangle: we don't send any corner data
> to the GPU. Instead the vertex shader is run 3 times, and each run gets a number
> `0`, `1`, `2` (`vertex_index`). We use that number to look up a hard-coded corner.
> So "the data" lives in the shader. Nothing to upload.
...
```rust
//! Render-pipeline constructors. Shader sources are compiled into the binary with
//! `include_str!`, whose path is relative to THIS file: `../../shaders/` = `src/shaders/`.
...
- `set_pipeline` — picks which recipe this pass uses.
- `draw(0..3, 0..1)` — `0..3` runs the vertex shader for `vi = 0, 1, 2`; `0..1` means
  one copy. The `LoadOp::Clear` already in the descriptor still paints the grey
  background *first*, then the triangle is drawn on top.

> The method is still called `clear` — that's fine for now. When the viewer draws
> many things you'll rename it `render`/`draw`, but keeping the name means `state.rs`
> needs no change this chapter.
