...
> Lesson [117](117-id-buffer-picking.md) re-runs this frame's whole draw list against a second set
...
| `Bounds`, `Aabb64` | nowhere — written out longhand | **`src/math.rs`** | anyone (lessons 64, 65, 70, 110, 113, 115) |
...
lines — lessons 64, 65, 70, 110, 113 and 115 all pass a box around and spell it out longhand.
...
mid-session, and lesson 117's id pass re-runs the draw list against a second set.
...
pub struct PipelineDesc<'a> {
    ...
    /// error message will hand back, so it is also lesson 107's error-scope label.
    pub label: &'a str,
    ...
    /// own attachment (lesson 93's R8Unorm mask, lesson 117's R32Uint id buffer).
