...
> Lesson [118](118-id-buffer-picking.md) re-runs this frame's whole draw list against a second set
...
| `Bounds`, `Aabb64` | nowhere — written out longhand | **`src/math.rs`** | anyone (lessons 65, 66, 71, 111, 114, 116) |
...
lines — lessons 65, 66, 71, 111, 114 and 116 all pass a box around and spell it out longhand.
...
mid-session, and lesson 118's id pass re-runs the draw list against a second set.
...
pub struct PipelineDesc<'a> {
    ...
    /// error message will hand back, so it is also lesson 108's error-scope label.
    pub label: &'a str,
    ...
    /// own attachment (lesson 94's R8Unorm mask, lesson 118's R32Uint id buffer).
