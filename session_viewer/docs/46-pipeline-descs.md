...
> Lesson [119](119-id-buffer-picking.md) re-runs this frame's whole draw list against a second set
...
| `Bounds`, `Aabb64` | nowhere — written out longhand | **`src/math.rs`** | anyone (lessons 66, 67, 72, 112, 115, 117) |
...
lines — lessons 66, 67, 72, 112, 115 and 117 all pass a box around and spell it out longhand.
...
mid-session, and lesson 119's id pass re-runs the draw list against a second set.
...
pub struct PipelineDesc<'a> {
    ...
    /// error message will hand back, so it is also lesson 109's error-scope label.
    pub label: &'a str,
    ...
    /// own attachment (lesson 95's R8Unorm mask, lesson 119's R32Uint id buffer).
