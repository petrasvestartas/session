# 85 Copy, duplicate, array — daily-use editing, nearly free
...
<svg viewBox="0 0 680 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="copy duplicates the selection minting fresh guids, inserts the batch into each source doc as one AddGeometry command, then places every copy with apply world delta" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <text x="340" y="100" fill="#888" text-anchor="middle">array = the same pipeline in a loop; Alt+gumball = the same pipeline on 59's release</text>
...
`Geometry` variants hold **`Rc<T>`** (56). So `lookup.get(guid).cloned()` clones the *handle*: the
...
```rust
    ...
    /// `one`, plural. Each snapshot inserts through 62's add_object path into ITS OWN doc.
...
```rust
    /// Doc-resolved duplicates of the selection, in 56's snapshot shape — ready for ONE
...
their rows resolve through `guid_to_row`, and each gets 59's commit primitive plus the live GPU
poke:
...
`CopyCmd` is 62's `LineTool` shape exactly — a struct with `from: Option<Point>`, the first click
stashes it, the second runs the batch: duplicate → ONE `AddGeometry` (the insert routes each
snapshot through 62's `add_object` into its own doc and `set_xform`s the carried local — the copy
...
(Snap (64) applies to both picks automatically, so copy-from-corner-to-corner is *exact*.)
...
```rust
        // 59's release handler, first lines:
        if let Some(ctx) = self.gb_drag.take() {
            ...
            // …the normal commit continues unchanged (59's apply_world_delta + TransformObjects)…
        }
```
...
- **Alt+drag** the X arrow → the original stays put, release → a copy at the drop point, one undo
  step. Without Alt → 59's normal move, unchanged.
...
Ch 85: DUPLICATION = three rails composed: duplicate() + AddGeometry + apply_world_delta. The trap:
       ...
       59's Esc discipline, second customer), copies take the final delta.
```

Edited: `app/history/add.rs` (`AddGeometry::of_snapshots`), `app/scene.rs` (`clone_selection`),
`app/commands.rs` (`copy`, `array`), `state.rs` (`place_copies`, Alt branch on 59's release).

## Next

`86-layers.md` — organization users expect: layers — which the PDF importer already built as tree
