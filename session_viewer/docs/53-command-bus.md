...
/// What the interface is currently waiting for. Idle = clicks mean selection (50);
...
pub fn dispatch(state: &mut State, line: &str) -> Dispatch {
    ...
    match verb {
        "hide" | "h" => {
            ...
            state.scene.hide_selected(&mut state.gpu);              // 51's verb, now bus-born
...
> forgot. The archive shipped exactly this shape. When commands become objects with options (54),
> the arms construct them — the dispatch point doesn't move.

## Step 3 — the CLI line: `src/ui/cli.rs` (NEW)

A bottom panel: a one-line log above an input field. It only *collects* the submitted string — 52's
...
1. In `src/ui/mod.rs`, add `mod cli;` at the top and give `UiState` (52's struct) four new fields:
...
and to the `UiState { … }` literal in `State::new` (52):
...
2. Inside 52's `build_ui` closure — after the `settings` window — call the panel and stash its
...
3. After `build_ui` returns, drain it into the bus — in `State::render()`, right after 52's
...
`State::on_left_release` (50), right after the `ray`/`tol` construction and BEFORE the
...
- **`zoom`** ⏎ → same as pressing F. **`frobnicate`** ⏎ → `unknown: 'frobnicate'  (type 'help')` —
  friendly, no panic.
- Typing in the CLI must **not** orbit the camera or trigger H/F shortcuts — that's 52's `consumed`
...
- The Get-loop plumbing (prompt display, click-feeds-point, Esc-cancels) is wired but no verb uses it
  yet — 54's multi-step command is its first customer. The `return` before selection logic is the
  contract to remember: **while a command runs, clicks belong to the command.**

## Recap

```
Ch 52: egui overlay — panels, input contract.
Ch 53: THE BUS. commands::dispatch(state, line) — a match IS the registry, aliases are patterns;
       instant verbs act and log (hide/show/zoom — 51's Scene verbs, now bus-born); multi-step verbs
       ...
       collect-then-apply.
       From here on, EVERY mutation is born as a command — undo (56) rides this for free.
```
...
`54-command-options.md` — commands grow **options** rendered in the prompt line, Rhino-style:
