# 75 GPU curves — the segment table gets a compute producer
...
Lesson 45's CPU arm costs `samples × point_at`, single-threaded, on every rebuild. The GPU has
...
`76-gpu-surfaces.md` — the same producer pattern aimed at the vertex arena: tensor-product
