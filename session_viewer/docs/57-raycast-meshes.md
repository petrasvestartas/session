    ...
    pub fn pick_ray(&mut self, ray: &Ray) -> Option<crate::app::pick::PickHit> {
        ...
        for (row, guid) in cands {
            ...
            let hit = match self.docs[d].session.lookup.get_mut(&guid) {
                ...
                _ => None,   // Line/Polyline/Point/PointCloud → lesson 59 (thin geometry needs a
...
lesson 54 with the other thin geometry.
...
`58-subobject-picking.md` — a hit tells you *which mesh*; sub-object picking tells you *which part*. From
