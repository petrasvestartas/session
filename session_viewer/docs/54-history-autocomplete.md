# 54 History & autocomplete — the CLI grows muscle memory
...
> This is a small lesson by design — three quality-of-life features on 52's CLI, no new architecture
...
52's `match` arms embedded the aliases in patterns; completion needs them *as data*. One pair of
...
and at the top of `dispatch`, replace 52's `let verb = parts.next().unwrap_or("");` with:
...
```rust
    pub cli_history: Vec<String>,        // ← ADD to State (init empty in State::new)
                                         //   NB: named cli_history, not history — lesson 55 adds
...
Now replace 52's `cli_panel` wholesale with the version below. Note the `history: &[String]`
...
Inside 52's `build_ui` closure (where `cli_panel` is already called and its return stashed in
...
```
Ch 53: options + multi-step — the conversation grammar.
Ch 54: MUSCLE MEMORY. VERBS + ALIASES as const tables — one source of truth for dispatch (resolve()
       before the match), Tab-completion, and help. History: cli_history: Vec<String> on State
       (named to dodge 55's undo `history`), pushed by run_command; the CLI walks it with ↑/↓ (cursor Option<usize>, live line stashed and restored
...
`55-delete-undo.md` — the first *destructive* mutation, and with it the pattern the whole editor
