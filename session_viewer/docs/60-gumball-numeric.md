# 60 Gumball V — click a handle, type a number

> **Big picture.** *Phase 9.* Dragging is for roughing; CAD work is exact. Rhino's gumball has a
> beloved shortcut: *click* (don't drag) an arrow, type `500`, Enter — an exact 500 mm move. 58's
...
```rust
    if let Some((handle, buffer, pos)) = &mut ui_state.gb_input {
        ...
        // State applies after the closure (51's rule)
...
**Open** — in the mouse-release handler. A subtlety: 58's commit block has already `take()`n
`gb_drag` by this point, so "was this a drag?" must be read *before* it. Insert
`let dragged = self.gb_drag.is_some();` as the first line of the release handler (above 58's
`if let Some(ctx) = self.gb_drag.take()`), then find 58's trailing `self.gb_pressed = None;` →
replace it with:
        ...
        if self.ui.gb_submit {
            ...
            if let Some((handle, buffer, _)) = self.ui.gb_input.take() {
                ...
                if let (Ok(v), Some(c)) =
                    ...
                    self.apply_transform_command(&delta);            // 58's commit path, factored:
...
`apply_transform_command` is 58's release-commit path as a named method: snapshot the current
selection's PLACEMENTS, `apply_world_delta` each row, wrap `before`/`after` in `TransformObjects`,
execute. It works for the drag release too, because 58's live path never mutates durable state — a
...
```rust
    ...
    /// (58's path: L' = L·W⁻¹·D·W per row — placement-only, kind-agnostic, place-conjugated).
    pub fn apply_transform_command(&mut self, delta: &Xform) {
        ...
        for s in &before {
            self.scene.apply_world_delta(s.row, delta);                   // 58's commit primitive
        }
...
Then shrink 58's release arm onto it — find the whole `if let Some(ctx) = self.gb_drag.take() { … }`
block (58's bake + Command code) → replace with:
...
1. **The lmb gate.** Track `lmb_down` from the *raw* winit `MouseInput` **before** egui routing —
   ...
   start a "no-button drag" the next time the cursor crosses the gumball. 58's threshold check
...
3. **The Escape guard.** Esc already means "cancel command" (52) — and in some setups "quit". While
...
```
Ch 59: rotate + scale — the drag family complete.
Ch 60: NUMERIC ENTRY. Release under 58's 4 px threshold = CLICK → egui popup at the cursor titled by
       ...
       scale floored 0.01 like drags) and commits through 58's exact path — undoable for free. The
       ...
       guard arm ABOVE 52's cancel arm (popup closes, nothing else reacts). The gumball is complete.
```
...
`61-draw-tools.md` — creating geometry at last: the `Tool` trait (ARCHITECTURE's pattern (b)),
