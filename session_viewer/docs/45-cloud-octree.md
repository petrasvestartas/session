# 45 Cloud octree — Potree's LOD, on our splat lane
...
spacing-limited SUBSAMPLE of the cloud, and the walk uploads points in octree order so
every node is one contiguous `(first, count)` range — which is exactly what a splat record
already is. Per frame a screen-error walk picks nodes: far clouds stop at coarse ancestors
(a few big dots), near clouds descend to raw leaves. Zoomed out a 13.8M-point scene costs
tens of thousands of points; zoomed in you get every point back, pixel-exact.

Streamed clouds (lesson [44](44-streaming-cloud.md)) keep their whole-cloud record: their
points never exist on the CPU, so there is nothing to build a tree from.
