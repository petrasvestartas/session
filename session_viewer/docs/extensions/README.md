...
The eleventh integrated checkpoint adds independent face and edge display colors, an Original reset in the picker, sparse mesh drag previews, and placement-aware point-cloud raster caching. `node tests/color-channels.cjs` captures `extensions-color-channels.png` with the downloadable `split.pb` shell: red faces, blue edges, and the color popup open. Its assertions also reset both channels independently and reopen the saved session. `node tests/large-object-dragging.cjs` measures the bundled bunny and lion with real pointer events.
