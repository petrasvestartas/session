# 70 Render-on-demand — the biggest win never touches the image
...
| selection & hover changes | 49's gestures, 57's gumball hover |
| any Command executed / undone / redone | `commit()`, the undo/redo verbs (55) |
| live gumball drag | `set_live_model` callers (58) |
| ghost preview updates | `set_preview` / `clear_preview` callers (62) |
| reconcile applied (watch/reload) | `apply_session` (44) |
| settings toggles, thickness slider | the apply-intent block (51) |
...
respect it or the text caret freezes. In `render()`, right after 51's
...
```rust
    pub fn render(&mut self) -> anyhow::Result<()> {
        ...
        // …the entire existing frame: cull (41), UI build (51), gpu.clear(…) — unchanged…
...
(Sweep the table's rows to call `poke()`. The watch poll (44) and 43's debounce still need ticks
...
1. `UiState` (51) gains `pub frames_drawn_last_sec: u32,` (seed `0` in `State::new`'s `UiState`
...
```rust
        ...
        self.ui.frames_drawn_last_sec = self.frames_drawn_last_sec;   // feed the HUD, 51's step 1
```

3. In `build_ui` (51), find the perf window's first label —
...
```
Ch 69: the stage — analytic ground.
Ch 70: RENDER-ON-DEMAND. Games render time; CAD renders STATE. One dirty flag; poke() = dirty +
       ...
       0 idle, display-rate while interacting. Static scenes now cost zero GPU; 71's AO rides on
...
`71-gtao.md` — ambient occlusion, the constant-quality way: half-resolution GTAO with a fixed tap
