# 53 Command options — modal, multi-step, Rhino-style
...
before the text ever reaches the command. First widen 52's `use` line so `CmdOption` is in scope:
...
```
Ch 52: the bus — verbs dispatch, instant commands work end to end.
Ch 53: THE CONVERSATION. CmdOption { Toggle / Number / List } renders into the Rhino prompt shape —
       ...
       `x,y,z` converges on feed_point, empty Enter = default, Esc = cancel (52). Geometry-free on
       purpose — 61's real tools copy the shape and only swap what Done does.
```
...
`54-history-autocomplete.md` — the CLI grows muscle memory: ↑/↓ recall previous commands, Tab
prefix-completes verbs, and an alias table (`l` → `line`) formalizes what 52's match patterns started.
