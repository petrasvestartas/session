...
ink=$(VIEWER_W=1400 VIEWER_H=900 $B "$out/local.ppm" assets/view_local.yaml 2>&1 | grep -o "non-background pixels: [0-9]*" | grep -o "[0-9]*$")
...
probe() {
    ...
    env VIEWER_W=1400 VIEWER_H=900 VIEWER_NO_EDGES=1 VIEWER_ZOOM=$z "$@" $B "$out/$name$z.ppm" "$out/plate.yaml" >/dev/null 2>&1
