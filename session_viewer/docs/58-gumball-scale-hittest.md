# 58 Gumball II — constant size + pickable handles
...
In `src/state.rs`, add to `impl State` (next to `refresh_gumball`, 57); the `proj_y`/`ortho_h`/
`vp_h` lines are 49's pick-site trio verbatim, and the camera fields (`unit`, `orientation`,
...
```rust
    ...
    /// distance (the archive's orbit-breathing bug). Mirrors 49's world_per_pixel per projection.
    fn gumball_scale(&self, origin: [f64; 3]) -> f32 {
        let unit = self.camera.unit.to_meters();                            // mm → m
        let depth = self.gumball_depth(origin);
        let proj_y  = 1.0 / (30.0_f64).to_radians().tan() * unit;           // 49's trio, verbatim
...
(`use session_rust::Vector;` if `state.rs` doesn't import it yet; the origin is 57's f64
`selection_centroid` — no casts needed here.) In `refresh_gumball` (57), find the
...
```rust
        ...
        // frame (selection changes already refresh at their own sites, 57; under 71's
...
allocations in 57's `upload_gumball` — reuse scratch vectors. Add `gb_seg_scratch:
...
The build already tags every row with its `HandleKind` (57). Hit-testing is: distance from the pick
...
```rust
/// Distance-based handle pick. `origin` = the widget's world anchor (57's geometry is
/// gumball-local — the ray is translated, never rebuilt). `tol` in world units
/// (49's world_per_pixel at the GUMBALL's depth × ~20 px — handles are small, be generous).
...
The two distance helpers are the classic closest-approach formulas, in f64 against 46's
...
`gumball.rs` (below 57's `use crate::engine::gpu::…` line). ~460 primitives, run per mouse event,
...
The input pecking order grows one level. From top: egui (52) → Get-loop (53) → **gumball** → scene
picking (47–50). `ray` here is 47's `engine::pick::screen_to_world_ray(&vp, &origin, self.cursor,
viewport)?` result (built early, so it's in scope). `proj_y`/`ortho_h`/`vp_h` are 49's three pick-site
...
One more ordering consequence: 52's marquee rect keys off `lmb_down` + drag distance, and a gumball
...
And the release half of the same gate: a press that landed on a handle must never reach 50's
selection branch (clicking an arrow would otherwise *also* select the object behind it). In
`on_left_release` (50), after 53's Get-loop reroute and before the click-vs-marquee branch:

```rust
        if self.gb_pressed.take().is_some() { return; }   // a handle ate this click (58)
```
...
This is the cursor-move handler, **not** the press handler — 49's `proj_y`/`ortho_h`/`vp_h` locals
...
46's `(f64, f64)`) and `gb_hovered: Option<HandleKind>` to `struct State`, and initialize **both to
`None`** in `State::new`. `state.rs` needs `use crate::engine::gumball::HandleKind;` for the field
types.

The tint itself is three edits. In `gumball.rs`, find 57's `build` signature:
...
Finally, in `refresh_gumball` (57, with Step 1's scale lines already in), the `build(…)` call gains
the new argument:
...
mod tests {
    ...
    fn ray(o: [f64; 3], d: [f64; 3]) -> crate::engine::pick::Ray {
        crate::engine::pick::Ray { origin: Point::new(o[0], o[1], o[2]),
                                   dir: Vector::new(d[0], d[1], d[2]) }   // unit d, as 46's is
    }
...
```
Ch 57: the widget exists — geometry, overlay pass, centroid anchor.
Ch 58: USABLE. Scale = SCREEN_PX(140) · world_per_px(depth) / ARC_RADIUS with depth = VIEW-SPACE Z
       ...
       GUMBALL → scene;
       a grabbed handle never leaks a click to selection — and never draws 52's marquee rect
...
`59-gumball-translate.md` — the first real transform: press an arrow, drag along its axis, watch the
