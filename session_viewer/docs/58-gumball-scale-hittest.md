...
(`use session_rust::Vector;` if `state.rs` doesn't import it yet.) In `refresh_gumball` (57), find
...
```rust
        if self.gb.is_some() { self.refresh_gumball(); }   // depth changed with the camera (58)
```
...
```rust
/// Distance-based handle pick. `tol` in world units (49's world_per_pixel × ~20 px — handles are
...
picking (42–45). `ray` here is 47's `engine::pick::screen_to_world_ray(&vp, &origin, self.cursor,
...
Finally, in `refresh_gumball` (57), the `build(…)` call gains the new argument:
