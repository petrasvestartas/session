# 5 · Build the nested session and graph panel
...
![Ownership and data flow](illustrations/README-01.svg)

Browse nested groups, select their visible objects, and hide or show their descendants. Graph attribute rows control vertices or both endpoints of matching edges. The panel is DOM; only highlights and visibility flags go to wgpu. Apply this step after the preceding integrated step; these edits include its shared wiring.
...
**Does the panel copy geometry?** No. It caches row references and subtree ranges. A parent selects or hides its descendants by range. A revision change rebuilds the index.

**What does a graph edge select?** The objects at both matching endpoints. Groups support selection and visibility; transforms still require one selected object.

**What bounds the work?** At most 200,000 index nodes and 1,000,000 row references, with 128 visible rows per page. An oversized document shows a capacity message instead of an incomplete group.
...
Expected compiler result: `Finished` with no errors. Open <http://localhost:8780/?data=off&inspect=1>. Create two lines. Press **L** and expand **Created**. Select its group: both lines highlight. Hide the group: both disappear. Show it: both return. This checkpoint uses DOM buttons; the next one replaces their presentation with egui.
