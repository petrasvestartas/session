# 77 Gumball IV — rotate and scale
...
For `RotateX`: `n = X`, `a_dir = Y`, `b_dir = Z` — the same `(i+1)%3 / (i+2)%3` pairing 65's arc
...
because the drag math works in radians, and 69's typed entry passes `true`. A bool at a call site
...
67's code untouched. That's the skeleton paying off. (`state.rs` gains `Line` in its
`session_rust` use for the axis-line constructor; `gumball.rs` needs nothing new — 67 already
...
**Fix 1 — preserve the sign.** The axis-scale spheres sit on the **negative** axis (65). The naive
...
Axis scale reuses 67's `closest_param_on_axis` for `t`; uniform scale measures the ray's hit
...
every group ends at the same two lines — 67's batched `set_live_models`, `TransformObjects` on
release.

**3a. The stash.** Add `a0: f64,` and `d0: f64,` to 67's `DragCtx` struct. In `begin_drag` (67),
find the `let t0 = …unwrap_or(0.0);` line → insert after it (the `use HandleKind::*;` above it
already covers these names). Like `t0`, these are PRESS references — 67's Step-1 aside is the why:
...
**3b. The dispatch.** In 67's mouse-move handler, find the whole
...
```rust
        ...
        if let (Some(ctx), Some(ray)) = (&self.gb_drag, self.cursor_ray()) {
            ...
            if let Some(delta) = delta {
                // 67's batched live path: stage all rows into the scratch, ONE write_buffer
                self.gb_edits.clear();
...
(The `if let Some(delta) = live_delta { … }` stash + `refresh_gumball_at` lines after it are 67's,
unchanged. So are 67's guards: the release's identity check skips no-op commits for every group,
since all four funnel into the same `last_delta`.)

The commit path does not change **at all** — the release still hands the final delta to 67's
...
**world-space**, and 67's helper conjugates it by the doc's `place` internally — for translate that
...
silently — add to 66's `#[cfg(test)] mod tests`:
...
```rust
    ...
    fn axis_scale_ratio_preserves_sign_and_damps() {
        // the scale spheres sit on the NEGATIVE axis (65): press t is negative, and the
...
```
Ch 67: translate — the drag skeleton (defer, live matrix, Command commit).
Ch 68: ROTATE + SCALE = two delta functions on that skeleton. Rotate: ray ∩ arc plane (denom guard
...
`78-gumball-numeric.md` — the click that 67's threshold deliberately left free: click a handle
