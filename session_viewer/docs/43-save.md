# 43 Save — write the file back, only when something changed
...
> edits to settle → write only if something truly changed** is the standard pattern, and 42b's
...
The fold lives in 42b's `content_hash(geom, &Xform)`, and the frame MUST match 42b's baselines
...
```rust
    ...
    pub fn save_if_changed(&mut self, doc: usize, filename: &str) -> Option<Vec<u8>> {
        ...
        // SAME frame as 42b's baselines: composed world placements, one downward pass.
...
```
Ch 42: reconcile — read a file in, diff by guid, touch only what changed.
Ch 43: SAVE — write ONE doc's file out (docs[0] here; active-vs-all is policy), THREE gates before
       ...
       no JS timer); (3) hash gate: save_if_changed re-hashes each dirty object against 42b's
...
`44-watch.md` — the third sync direction: an *external* edit to the file flows back in. The browser can't
watch a filesystem, so a File System Access handle polls `lastModified` (or a watcher→WebSocket bridge
pushes), and on change we run 42's `reconcile`. The catch is the loop: our own Step-4 save changes the
