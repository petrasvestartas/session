# 75 Tree ↔ viewport — one selection, two views
...
> clicks route through 49's verbs (74), so tree → viewport is free. This lesson closes the loop the
...
<svg viewBox="0 0 680 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="clicking an object in the viewport expands its ancestor chain in the tree and scrolls the row into view; both views show one selection" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <text x="530" y="77" fill="#666" font-size="10">highlight (49's set)</text>
...
On a successful viewport pick (42/48's `pick_ray`, right after the selection update in 49's click
...
The scroll request is *input* to the panel (State → panel), so it can't ride on 74's `TreeIntent`
(that flows panel → State). Give `tree_panel` one more parameter — extend 74's signature with
...
from 74's Step 2 and replace it with this):
...
problem mirrored. Zoom *is* a genuine intent (panel → State), so it rides on 74's `TreeIntent` — add
...
```rust
    ...
    if let Some(g) = intent.zoom_to {
        let (lo, hi) = self.scene.world_aabb(&g);                      // 41's helper
...
```
Ch 74: the tree renders state.
Ch 75: THE LOOP CLOSED. Viewport pick → reveal_in_tree: find the owning Doc, get_node_by_name
       ...
       tree_scroll_to, poke. The panel resolves it AFTER flatten (same-frame ordering via 51's data
...
`76-text-labels.md` — names in the 3-D view itself: billboarded text from a glyph atlas, readable at
