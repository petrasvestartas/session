...
pub struct SnapContext<'a> {
    ...
    pub world_per_pixel: f64,    // at the raw point's depth (49's trio, computed by the caller)
}
...
`query_box_around` builds the broad-phase world `OBB` around `raw` and reuses 40's `objects_in` —
...
> actually changes when: the candidate *set* changes only with the scene (43b's reconcile
> generation), their *screen positions* only with the camera (71's dirty flag). If profiling ever
...
```rust
        let raw = if let Some(hit) = self.scene.pick_ray(&ray, tol) {
            ...
            // empty space: intersect the ground plane z = 0 (63, unchanged)
...
Ch 64: PRECISION. app/snap.rs: SnapKind { Endpoint/Vertex(0) > Grid(6) } — RANK first, pixel
       ...
       frame or you snap to nowhere), gathered via 40's BVH around the raw point; the broad-phase
