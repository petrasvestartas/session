# 86 Layers — naming what the importer already built
...
> group per OCG layer, per doc** (pdf.rs — `add_group` per named CAD layer), 75's panel renders
> them with eye icons, and branch visibility resolves to 51's set operations. What's missing is
...
75's identity rule decides everything here. In the kernel tree, `node.name` is the document
...
```rust
    ...
    /// fed to 51's hidden set. Object guids live in node.NAME (75's identity rule).
...
```rust
        "layer" => match (parts.next(), parts.next()) {
            ...
            (Some(dir @ ("off" | "on")), Some(name)) => {
                // fold over the docs, union the descendants, hand them to 51's set ops
...
- `layer list` → the *imported* layers, by name, with member counts — Bemassung, Schraffur, … —
  exactly the groups 75's panel shows per sheet, now summed across docs. Nothing was created;
  the import did this.
- `layer off Bemassung` → dimensions vanish from **every sheet at once** — draw AND pick (the
  cross-doc fold feeding 51's one authority). `on` → back. 75's per-sheet branch eyes still work
  independently — same `hidden` set, two entrances. Undo history untouched — visibility isn't a
  document mutation (51's rule).
...
- Save (44) → reload → the layer structure round-trips **per doc**: each doc's `.pb` carries its
...
`87-measure.md` — `probe` grows up: distance, angle, radius, object info — and a status bar that
