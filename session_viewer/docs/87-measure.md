...
Each is 54's template with a different `Done`. Build each as a `ProbeCmd`-shaped struct (54) that
...
Every point in these conversations comes through `cursor_world_point` — **snapped** (64). Measuring
...
nothing needs snap, resolve the readout against the work plane only (80's one intersection) —
...
`88-dev-toolbox.md` — the developer-experience lesson: a headless selftest, GPU errors that surface
