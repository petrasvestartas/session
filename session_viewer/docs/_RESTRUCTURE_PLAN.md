...
| the splat lane | `Splat` (per-pixel buffers, resolve) + one `SplatSlot` (records + 2 bind groups) **per point lane**, inside `CloudLane`; `Gpu::splat_records(&self, draws: &[CloudDraw]) -> ([u32; 4], Vec<u8>, u32)` keeps that exact signature | lesson 43 (streaming) adds `stream.rs` as a second lane owning its own slot; lesson 44 (octree) anchors on `splat_records` |
