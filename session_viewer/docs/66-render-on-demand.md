# 66 Render-on-demand — the biggest win never touches the image
...
```rust
    pub fn render(&mut self) -> anyhow::Result<()> {
        ...
        // …the entire existing frame: cull (37), UI build (47), gpu.clear(…) — unchanged…
...
```
Ch 65: the stage — analytic ground.
Ch 66: RENDER-ON-DEMAND. Games render time; CAD renders STATE. One dirty flag; poke() = dirty +
...
`67-gtao.md` — ambient occlusion, the constant-quality way: half-resolution GTAO with a fixed tap
