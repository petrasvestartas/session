# 4 · Draw a solid, readable gumball
...
![Ownership and data flow](illustrations/extend-gumball.svg)

![Unlit cylindrical gumball in the maintained viewer](screenshots/extensions-gumball.png)

Replace the flat strokes with actual triangle meshes: cylindrical shafts, cone tips, round rotation tubes and flat-color spheres. Build vertices once and render an antialiased, bounded tile independently of the scene sample count. Apply this step after the preceding integrated step; these edits include its shared wiring.
...
**What is a mesh here?** A fixed list of triangles forming cylinders, cones, spheres and rotation tubes. The CPU builds it once; a GPU vertex buffer retains it.

**Why is there no shading?** The fragment shader outputs the handle color directly. It has no light or normal calculation.

**Why are edges smooth?** Draw into a tile at twice the display resolution with four samples per pixel, then blend its coverage over the scene. Deselect destroys that tile; the fixed mesh stays reusable. The tile is capped at 1024×1024.

**How do I pick a face under a handle?** Hold Ctrl for edges or Ctrl+Shift for faces. The input handler bypasses control and gumball grabs while Ctrl is held.
...
Expected compiler result: `Finished` with no errors. Open <http://localhost:8780/?data=off&inspect=1>. Create and select a line. Press **7** for isometric view. Cone tips and cylindrical shafts have flat colors and smooth edges. Drag a shaft to move, a colored sphere to scale an axis, a ring to rotate, or the center sphere to scale uniformly. Undo each operation. Deselect: handles disappear and their temporary tile is released.
