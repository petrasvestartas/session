# 100 Section planes — cut the building open
...
<svg viewBox="0 0 680 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="a section plane cuts the model; fragments on the negative side are discarded per pixel; the plane is set by three points or the work plane and dragged along its normal" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(330,24)">
    ...
    <text x="0" y="58" fill="#d7dae0">set: 3 points · work plane (87)</text>
    <text x="0" y="78" fill="#d7dae0">move: drag = 67's axis math</text>
    <text x="0" y="98" fill="#d7dae0">pick: hits behind the cut rejected</text>
    <text x="0" y="122" fill="#666" font-size="10">count = 0 → zero cost; planes rebased like 53's frustum</text>
  </g>
...
```rust
    ...
    pub fn upload_sections(&mut self, sections: &[Plane], anchor: &Point) {
        ...
        for (i, pl) in sections.iter().take(4).enumerate() {
            ...
            // rebase to anchor-relative: n·(p−o) + (d + n·o) == n·p + d  (53's identity, reused)
...
`section` composes machinery you already have — 62's multi-step points, 87's work plane, 67's axis
drag:
...
```rust
        "section" => match parts.next() {
            ...
            _ => { let (cmd, get) = SectionBy3Points::start();   // 87's 3-point flow, verbatim
                   Dispatch::Start(cmd, get) }
        }
```

`SectionBy3Points` is 87's `Cplane3Points` **verbatim** except its last two lines — copy that
...
The drag is 67's skeleton pointed at a plane instead of an object — live `poke()` per move,
...
(Both commands live in `commands.rs` beside 87's; `state.cursor_ray()` goes `pub(crate)` so they
can call it — one-word edit in `state.rs` (67). `Plane::from_point_normal(point, normal)` and
...
Ch 83: SECTIONS. Up to 4 world planes in one uniform, rebased camera-relative per frame (d_rel =
       d + n·origin — 53's identity reused); every geometry fs runs one keep-test loop and discards
       ...
       never disagree. Command: `section` = 3 points (62), `wp` (87), off/flip; `section drag` =
       67's closest_param_on_axis aimed at the plane's own normal, live per move. Sections are
...
`101-import-export.md` — the kernel's OBJ codec (and one honest gap about STEP) reaches the browser:
