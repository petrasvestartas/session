# 61 Draw tools I — creating geometry
...
Adding and deleting are the same operation with time reversed — 55's `RemoveObjects` already knows
...
Two tiny additions to `RemoveObjects` in `src/app/history/remove.rs` (55) — add both inside its
`impl RemoveObjects`, beside `of_selection`:

```rust
    /// Wrap ready-made snapshots (61's AddGeometry; 84's copy batches reuse it).
...
destructure from 55's note, factored once):
...
```rust
    pub fn commit(&mut self, cmd: Box<dyn crate::app::history::Command>) {
        ...
        scene.rebuild_bvh();   // new/removed objects change the box set (40)
    }
```
...
impl ActiveCommand for PointTool {
    ...
    fn feed_text(&mut self, state: &mut crate::state::State, s: &str) -> CmdStep {
        // 'x,y,z' typed — same convergence as probe (53)
...
Structurally `ProbeCmd` (53) with a different `Done`:
...
Remember 52's empty-click rule — a click on bare grid intersects `z = 0` and still yields a point —
...
- **Save** (43) after drawing → reload the downloaded file → your drawn line is in it. It was a
...
```
Ch 60: numeric entry — the gumball is complete.
Ch 61: CREATION = pattern (b), and it's small on purpose: a drawing tool IS an ActiveCommand (53's
       machinery: prompts, click-or-type convergence, back, Esc) whose finish COMMITS an AddGeometry
       (55's machinery: undo). AddGeometry = RemoveObjects mirrored — apply calls revert and vice
       ...
       Session-first-class: they save (43), diff (42b), pick (46), and undo like everything else.
```
...
`62-draw-tools-2.md` — tools that need *N* clicks and live feedback: `PolylineTool` (Enter finishes),
