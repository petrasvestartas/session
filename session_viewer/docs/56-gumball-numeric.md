# 56 Gumball V — click a handle, type a number

> **Big picture.** *Phase 9.* Dragging is for roughing; CAD work is exact. Rhino's gumball has a
> beloved shortcut: *click* (don't drag) an arrow, type `500`, Enter — an exact 500 mm move. 54's
...
1. **The lmb gate.** Track `lmb_down` from the *raw* winit `MouseInput` **before** egui routing —
   ...
   start a "no-button drag" the next time the cursor crosses the gumball. 54's threshold check
...
3. **The Escape guard.** Esc already means "cancel command" (48) — and in some setups "quit". While
...
Ch 56: NUMERIC ENTRY. Release under 54's 4 px threshold = CLICK → egui popup at the cursor titled by
       ...
       guard arm ABOVE 48's cancel arm (popup closes, nothing else reacts). The gumball is complete.
```
...
`57-draw-tools.md` — creating geometry at last: the `Tool` trait (ARCHITECTURE's pattern (b)),
