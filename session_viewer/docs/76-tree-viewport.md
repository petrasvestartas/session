# 76 Tree ↔ viewport — one selection, two views
...
> clicks route through 50's verbs (75), so tree → viewport is free. This lesson closes the loop the
...
<svg viewBox="0 0 680 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="clicking an object in the viewport expands its ancestor chain in the tree and scrolls the row into view; both views show one selection" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <text x="530" y="77" fill="#666" font-size="10">highlight (50's set)</text>
...
The scroll request is *input* to the panel (State → panel), so it can't ride on 75's `TreeIntent`
...
from 75's Step 2 and replace it with this):
...
problem mirrored. Zoom *is* a genuine intent (panel → State), so it rides on 75's `TreeIntent` — add
...
Ch 76: THE LOOP CLOSED. Viewport pick → reveal_in_tree: find the owning Doc, get_node_by_name
       ...
       tree_scroll_to, poke. The panel resolves it AFTER flatten (same-frame ordering via 52's data
