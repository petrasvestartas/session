# 83 Import / export — OBJ in, OBJ out (and the STEP honesty)
...
> **Open… file dialog**, and `export obj` via 43's download path. It also triggered a kernel fix:
...
<svg viewBox="0 0 680 130" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="obj text enters via a manifest item or a file dialog, parses through the kernel string codec into a mesh, joins the session; export runs the writer and downloads" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <text x="340" y="104" fill="#888" text-anchor="middle">export = write_file_obj_to_string → 43's Blob download · STEP: C++-only today (kernel-gap #11)</text>
...
`scene.add_file` + `gpu.set_scene(&scene.tables)` — and, better, **the watch loop (44) and
reconcile (42b) work on OBJ files too**, because everything downstream only ever sees a `Session`.
...
*changed copy* of an already-loaded doc is a different operation: that's 42b's reconcile — match
...
The mirror, riding 43's download machinery. There is no "the session" anymore — a scene is
*several* docs, so `export pb|json` writes the **active doc** (61's `active_doc`;
...
```rust
        "export" => match parts.next() {
            ...
            Some("json") => {                                          // ONE doc, 43's machinery
...
```
Ch 82: sections.
Ch 83: FILES. Import: session_from_bytes_chunked grows a `.obj` early-return arm (the .json arm is
...
`84-copy-array.md` — duplication: `copy` between two points, Alt+gumball-drag-a-copy, and `array` —
