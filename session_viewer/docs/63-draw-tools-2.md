# 63 Draw tools II — polyline, rectangle, box, and the ghost preview
...
> A tool shows you a possible future; only `finish` makes it real. Get that boundary wrong and 44's
...
```rust
    // ghost preview table (63) — replaced whole on every tool mouse-move
...
```rust
            // ghost preview (63) — same pipeline, its own table at group 3. Fully self-bound:
...
In `state.rs`, the point resolver becomes a named method — 53's click reroute computed
"pick-or-z=0" inline; motion needs the same answer every mouse-move, so factor it now. Add to
`impl State` (59's `cursor_ray` sits right above it):
...
(53's `WaitingPoint` click branch now reads `if let Some(p) = self.cursor_world_point()` and feeds
...
`feed_text(state, "")` (53) — polyline uses that as "finish".
...
Register the verbs — in `dispatch()`'s match (`app/commands.rs`), beside 62's `"point"`/`"line"`
arms:
...
- The leak test: mid-polyline with 3 ghost points, hit **Save** (Ctrl+S if wired, or wait for 44's
  debounce) → the saved file contains *no trace* of the ghost. Esc → ghost gone, nothing committed.

## Recap

```
Ch 62: creation — tools are ActiveCommands, finish = AddGeometry.
Ch 63: N-CLICK TOOLS + THE GHOST. Preview = a dedicated small segment buffer (gumball's table
...
`64-snapping.md` — drawing becomes *precise*: endpoints, vertices, and grid intersections within a
