...
> **Big picture.** *Phase 10b closes.* A BRep (68) is faces + shared edge curves + topology.
> On the GPU it is **nothing new**: every face is a 69b grid trimmed by 69c's loops, every
> edge is a 69a curve into the segment table. This lesson is therefore mostly an *assembly*
...
<svg viewBox="0 0 680 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="a brep splits into per-face trimmed grids and shared edge curves; both faces clip against the same curve so the seam is covered; one dispatch per face, edges through the curve lane" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(250,22)" font-size="11">
    <text x="0" y="10" fill="#d7dae0">face k  → 69b grid + 69c loops   (one dispatch each)</text>
    <text x="0" y="32" fill="#d7dae0">edge j  → 69a segments           (m_curves_3d, real curves)</text>
...
```
src/app/scene.rs   # the BRep arm: per-face 69b/69c reservations, per-edge 69a reservations
...
68's arm drew `b.mesh()` from the cache. The GPU arm walks topology instead — for each face,
    ...
    Geometry::BRep(b) => {
        ...
        for f in 0..b.m_faces.len() {
            let (srf, loops) = brep_face_uv(b, f);      // surface ref + UV loop polygons,
                                                        // discretized like 69c (the pcurves
                                                        // live in b.m_curves_2d via m_trims)
            let (gu, gv) = grid_for(srf);
            // 69b reservation + index grid, 69c loop upload — verbatim from those arms
        }
        for c in &b.m_curves_3d {
            // 69a reservation: spans × 64 clamped, seg_base, FACING_UNKNOWN
        }
...
Read `BRep::mesh()` (brep.rs) once more with 69c eyes. Phase 1 classifies faces; phase 2
...
chosen sub-pixel — and (b) f32 eval noise, ~1e-5 of object scale (measured in 69a). Neither is
visible, and 68's real edge curves draw ON the seam with a pen width anyway. So the display
...
triangles (picking proxy included — 47 raycasts the CPU mesh, coarse, as in 69b).
...
snaps onto the loop polygon), noted in 69c and deliberately not built until a real model
demands it.
...
cadence from 69a governs all of it. There is nothing else — which is the lesson.
...
1. **Fidelity**: GPU build vs 68's CPU cache build, same camera — silhouettes and shading
...
```
Ch 69d: Phase 10b closes on wiring, not shaders: face = 69b grid + 69c trim, edge = 69a
...
`70-ground-grid.md` resumes the mainline — an *analytic* plane drawn without vertices, which
