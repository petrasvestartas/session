# 82 Section planes — cut the building open
...
<svg viewBox="0 0 680 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="a section plane cuts the model; fragments on the negative side are discarded per pixel; the plane is set by three points or the work plane and dragged along its normal" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(330,24)">
    ...
    <text x="0" y="58" fill="#d7dae0">set: 3 points · work plane (79)</text>
    <text x="0" y="78" fill="#d7dae0">move: drag = 58's axis math</text>
    <text x="0" y="98" fill="#d7dae0">pick: hits behind the cut rejected</text>
    <text x="0" y="122" fill="#666" font-size="10">count = 0 → zero cost; planes rebased like 41's frustum</text>
  </g>
...
> field is an **E0063** build error — same as 79's `work_plane`). Step 4's `state.sections…` code needs it.
...
**camera-relative** (33), so the planes rebase per frame exactly like 41's frustum did — same move,
opposite direction:
...
```rust
    ...
    pub fn upload_sections(&mut self, sections: &[Plane], anchor: &Point) {
        ...
        for (i, pl) in sections.iter().take(4).enumerate() {
            ...
            // rebase to anchor-relative: n·(p−o) + (d + n·o) == n·p + d  (41's identity, reused)
...
```rust
            wgpu::BindGroupLayoutEntry {
                binding: 1,                                    // sections (82) — every shader sees it
...
= cloud billboards; `ground.wgsl` = 69's analytic ground, which computes its own `hit`) gains the
...
`section` composes machinery you already have — 53's multi-step points, 79's work plane, 58's axis
drag:
...
```rust
        "section" => match parts.next() {
            ...
            _ => { let (cmd, get) = SectionBy3Points::start();   // 79's 3-point flow, verbatim
                   Dispatch::Start(cmd, get) }
        }
```

`SectionBy3Points` is 79's `Cplane3Points` **verbatim** except its last two lines — copy that
...
The drag is 58's skeleton pointed at a plane instead of an object — live `poke()` per move,
...
(Both commands live in `commands.rs` beside 79's; `state.cursor_ray()` goes `pub(crate)` so they
can call it — one-word edit in `state.rs` (58). `Plane::from_point_normal(point, normal)` and
...
```
Ch 81: the capstone loop.
Ch 82: SECTIONS. Up to 4 world planes in one uniform, rebased camera-relative per frame (d_rel =
       d + n·origin — 41's identity reused); every geometry fs runs one keep-test loop and discards
       ...
       never disagree. Command: `section` = 3 points (53), `wp` (79), off/flip; `section drag` =
       58's closest_param_on_axis aimed at the plane's own normal, live per move. Sections are
...
`83-import-export.md` — the kernel's OBJ codec (and one honest gap about STEP) reaches the browser:
