...
> **Big picture.** *Phase 10b — GPU tessellation (69a–69d).* Phase 10 put every curved type on
> screen by tessellating on the **CPU**: 65 sampled `point_at(t)` into a polyline, 66/68 cached
> `mesh()`, 69 ran the CDT. That is the correctness reference — keep it. This phase moves the
...
<svg viewBox="0 0 680 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="control points and knots upload once; a compute dispatch evaluates de boor per segment and writes cylinder segment rows into the shared table; both line lanes draw them unchanged" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <text x="340" y="110" fill="#888" text-anchor="middle">CPU polyline (65) stays: it is the PICK proxy and the acceptance reference</text>
...
Lesson 65's CPU arm costs `samples × point_at`, single-threaded, on every rebuild. The GPU has
...
CVs)` — no neighbor, no sequence. The whole of 65's `sample_curve` becomes one dispatch.

What does NOT move: the **pick proxy**. 47's raycast walks CPU-side geometry; a GPU-resident
polyline would need a readback to pick. So `sample_curve` (65) still runs — at a fixed coarse
...
fn tess(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    s.radius = 0.0;                    // screen-constant global pen, like 65's CPU arm
...
Honest label: this is 65's *uniform-per-span* sampling, not chord-error adaptivity — the GPU
...
```rust
/// The viewer's FIRST compute pipeline: curve tessellation (69a). Bind group 0 is built ad hoc
...
65's arm pushed `samples - 1` filled rows. Now it pushes the same COUNT of **zeroed rows**
...
```rust
        let samples = (nc.span_count() as u32 * 64).clamp(64, 2048);   // GPU headroom: 4x 65's
...
nothing — the contract render-on-demand (71) will rely on.

## What you should see

Draw a `curve` (65's tool) and it looks *identical* — that is the acceptance: **CPU and GPU
...
Then zoom close to a tight bend: where 65's 16-per-span polyline showed chords, the 64-per-span
...
```
Ch 69a: the pattern of Phase 10b, once: a COMPUTE PRODUCER writes a table the viewer already
...
`69b-gpu-surfaces.md` — the same producer pattern aimed at the vertex arena: tensor-product
NURBS evaluation in compute, one invocation per grid vertex, and 66's cache becomes a
