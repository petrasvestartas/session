# 79 Work plane — "the ground" becomes a choice
...
<svg viewBox="0 0 680 130" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="the work plane is stored state; the click resolver, grid, snap, and draw tools all read it; set by three points or a picked face" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g fill="#d7dae0" font-size="10">
    <text x="466" y="30">empty-click resolver (52) → ray ∩ plane</text>
    <text x="466" y="61">grid shader (69) → plane frame</text>
    <text x="466" y="92">rect/box (62), grid snap (63) → plane uv</text>
  </g>
...
`cursor_world_point` (52/63) swaps its hardcoded `z = 0` intersection for the plane:
...
A three-way verb, all Get-loop (53's grammar). The dispatch arm (`commands.rs` needs
...
The two commands, below `dispatch` in `commands.rs` — 53's ActiveCommand shape, nothing new:
...
impl ActiveCommand for CplaneFace {
    fn feed_point(&mut self, state: &mut crate::state::State, _p: Point) -> CmdStep {
        // the fed point is the snapped click; the FACE comes from 47's sub-object resolve
...
```rust
    /// The plane of the mesh face under the cursor (47's resolve → kernel face normal).
...
**`rect`/`box` (62) build in plane uv.** Corner clicks convert to plane coordinates, the rectangle
...
In `rect.rs` (62), find `rect_corners` → replace it and its callers' first lines:
...
**Ground/grid (69).** The analytic shader gains the plane frame; ray∩plane replaces ray∩z=0 (same
...
init `(Point::new(0.0, 0.0, 0.0), Vector::x_axis(), Vector::y_axis())` in `Gpu::new`) — and 69's
per-frame `GroundUniform` fill writes `plane_o = work_plane.0 − origin` (the camera-relative
subtract, f64 then cast) plus the two axes in place of the old `ground_z`. If you took 69's
fract/fwidth infinite-grid upgrade, its lines already key off the hit — express them in `uv` and
they ride the plane for free.

**Snap (63).** The `Grid` candidate quantizes in plane uv instead of world xy; `Endpoint`/`Vertex`
snaps are world-space and unaffected. `snap()` gains a `plane: &Plane` parameter (the call site in
`cursor_world_point` passes `&self.work_plane`; `snap.rs` imports `Plane`). Find 63's grid-candidate
block (`if raw[2].abs() < 1e-6 { … }`) → replace with:

```rust
    // grid candidate — the crossing nearest the raw point, in PLANE uv (79)
...
```rust
    /// The work plane changed: push the frame to the ground shader + redraw (70).
...
```
Ch 78: edit points — modeling on the curve.
Ch 79: CPLANE. work_plane: Plane (kernel type: origin + axes IS a construction plane), default world
       XY. The ONE empty-click resolver (48/63's cursor_world_point) swaps z=0 for ray∩plane — every
       ...
       render on the plane (frame uniform into 69's analytic shader); grid snap quantizes in plane
...
`80-advanced-perf.md` — headroom for scenes beyond the stress file: LOD/decimation, occlusion
