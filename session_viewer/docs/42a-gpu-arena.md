# 42a Reconcile I — a per-object GPU arena

> **Big picture.** *Phase 6 — the `.pb` file becomes the live source of truth, like a real CAD app.*
> The plan: reload only what changed (42b), save only when something changed (43), watch for external
> edits (44). None of that is possible while the GPU side is all-or-nothing — lesson 30's arena is one
...
//! fits. `slots` maps each guid to its ranges so reconcile (42b) can free/replace one object in
//! place.
...
> to re-upload untouched neighbours — the property reconcile (42b) needs, and the one a flat `Vec`
> can't give.
...
```rust
    /// One object's linework in/out by guid. 42b's reconcile calls these; set_scene bulk-fills
...
```rust
    ...
    // 42b's set_object_row, which defines grow_instances on this exact pattern (instances /
...
**returns** one object's tables (same body, different sink — 42b's `apply_object` reuses it), in
`scene.rs`:
...
```rust
        ...
        // A full refill per set_scene keeps 35's one-upload-path shape; 42b's reconcile is what
...
Ch 42a: PER-OBJECT ARENA. GpuArena (engine/gpu/arena.rs) replaces 30's flat arena: guid →
...
`42b-reconcile.md` — the arena can now touch one object, so the diff arrives: fingerprint every object
