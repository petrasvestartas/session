# 2 · Create, trim, extend and explode
...
![Ownership and data flow](illustrations/20-03.svg)

Type world coordinates, change exact kernel geometry, and keep each command as one undo step. No new shader is needed: the point and line lanes already draw these objects. Apply this step after the preceding integrated step; these edits include its shared wiring.
...
**Why no new shader?** Points and curves already have GPU draw paths. A command changes the CPU document; `Scene::rebuild` converts it into their existing upload rows.

**When does undo record?** Once per successful command. Validation runs before mutation. A polyline explosion removes one polyline and adds its lines inside one transaction.

**What does `trim 0.2 0.8` mean?** Keep 20% through 80% of the current curve. For a line from x=0 to x=100, the result is x=20 to x=80.
...
Expected compiler result: `Finished` with no errors. Open <http://localhost:8780/?data=off&inspect=1>. Press **:**, enter `line 0,0,0 100,0,0`, then **Enter**. Press **F**, click the line, reopen **:** and run `trim 0.2 0.8`. The middle 60% remains. Run `undo`: the full line returns. This checkpoint still has the original DOM command field.
