...
8. **`src/state.rs`** — the `gizmo` field (`src/state.rs`); `upload_gizmo` from `render` after `rebase_anchor` (`state.rs`); origin set or cleared in `select`, `clear`, `escape_selection`. **First visibly working point**, no interaction yet.
