# 42b Reconcile II — diff by guid, touch only what changed
...
impl Scene {
    ...
    /// 44's watcher generalizes the diff per doc.)
...
verbs, and the `Geometry` match — converting an object to those GPU types via 42a's `flatten_mesh` /
...
```rust
    ...
    /// rows. Same shape as 42a's ensure_seg_capacity; needs `instance_layout` hoisted onto Gpu.
...
The two `row == order index` consumers, made honest — in 40's `rebuild_bvh`, find:
...
and in 40's `objects_in`, `.map(|id| id as u32)` becomes
...
cargo test -p session_viewer reconcile --target x86_64-unknown-linux-gnu   # override the wasm pin (40)
```
...
```
Ch 42a: per-object arena — free/replace one object's GPU bytes without touching neighbours.
Ch 42b: THE DIFF. content_hash = hash of the kernel's SORTED jsondump + the object's session
...
`43-save.md` — the reverse trip. An in-viewer edit marks the object dirty; a debounce coalesces the
