...
Same caveat as 50's `selected`: guids can collide across docs, so a hide keyed by guid can catch a
...
```rust
    // pick_mesh (47) — first line inside `for (row, guid) in cands {`  (guid is an owned String → &guid)
    if self.hidden.contains(&guid) { continue; }

    // pick_thin (49) — before the `match` inside `for h in hits {`  (h.guid() already returns &str)
...
Ch 51: VISIBILITY. Scene.hidden (parked since 35) becomes runtime state with three consumers that
       ...
       50's flip-tracked upload — tables.objects[row].2 is the truth (hides survive later Msg::File
       ...
       clears. H / Ctrl+H until the CLI takes over (53). Phase 7 complete: ray → hit →
