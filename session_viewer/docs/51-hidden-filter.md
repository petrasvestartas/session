# 51 Hidden objects — visibility is real state
...
> respected by **drawing** (41's shader collapse), **picking** (47/49), and **marquee** (50) — three
> different code paths agreeing about one fact. It's also the first *verb* — `hide`/`show` here become
> the first commands the CLI dispatches in 53.
...
<svg viewBox="0 0 680 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="one hidden set feeds three consumers: the shaders skip drawing, the pickers skip hits, the marquee skips membership" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g fill="#d7dae0" font-size="10">
    <text x="314" y="27">draw — FLAG_HIDDEN → vs collapses to w=0 (41, already wired)</text>
...
```
src/engine/gpu/mod.rs   # set_flag_rows(flag, rows) — the generalization of 50's set_selected_rows
...
50's `set_selected_rows` and the visibility upload are the same function with a different bit. Extract
it — this is the moment the pattern earns a name (50's tables-then-live-row discipline comes along:
...
```rust
    ...
    /// tables first (the truth), then the live instance via write_row_flags (50). One function
    /// serves SELECTED (50) and HIDDEN (here) — CULLED (41) keeps its own per-frame path.
...
and 50's function becomes a wrapper (find `set_selected_rows`, replace its body):
...
The shader side needs **nothing**: 41 Step 3 already collapses any `FLAG_HIDDEN` row to a clipped
...
Unlike 50's `selected` (row-keyed), `hidden` stays a guid set — 35's `add_file` consults it by guid
...
```rust
    // pick_mesh (47) — first line inside `for (row, guid) in cands {`
    if self.tables.objects[row as usize].2 & Instance::FLAG_HIDDEN != 0 { continue; }

    // pick_thin (49) — right after its `let Some(&row) = self.guid_to_row.get(h.guid()) …` line
    if self.tables.objects[row as usize].2 & Instance::FLAG_HIDDEN != 0 { continue; }

    // select_marquee (50) — first line inside `for row in 0..self.world_boxes.len() {`
    if self.tables.objects[row as usize].2 & Instance::FLAG_HIDDEN != 0 { continue; }
```
...
```
Ch 50: selection state + marquee.
Ch 51: VISIBILITY. Scene.hidden (parked since 35) becomes runtime state with three consumers that
       must agree: draw (FLAG_HIDDEN → w=0 collapse, wired in 41 — zero new shader work), pick
       ...
       generalizes 50's flip-tracked upload — tables.objects[row].2 is the truth (hides survive
       ...
       H / Ctrl+H until the CLI takes over (53). Phase 7 complete: ray → hit →
...
`52-egui-hud.md` — Phase 8: the interface. An egui overlay lands on top of the 3-D pass — the perf HUD
