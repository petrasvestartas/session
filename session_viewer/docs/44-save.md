...
The fold lives in 43b's `content_hash(geom, &Xform)`, and the frame MUST match 43b's baselines
...
```rust
    ...
    pub fn save_if_changed(&mut self, doc: usize, filename: &str) -> Option<Vec<u8>> {
        ...
        // SAME frame as 43b's baselines: composed world placements, one downward pass.
...
Ch 44: SAVE — write ONE doc's file out (docs[0] here; active-vs-all is policy), THREE gates before
       ...
       no JS timer); (3) hash gate: save_if_changed re-hashes each dirty object against 43b's
