# 67 Iso-curves — the lines that make a surface read
...
> the same recipe as 65's curve body, evaluated on the surface.
...
```
src/app/scene.rs   # iso extraction beside the surface build arm (66); cached with the tessellation
```
...
```rust
    ...
    /// 66's surface_mesh, widened: the cache entry is now (mesh, linework) — both built once.
...
- Gumball-drag the surface: lines and skin move as one (same row, same placed frame — local-space
  linework, Step 1's parenthetical) and the perf HUD stays flat (cached, 66's rule).

## Recap

```
Ch 66: surfaces — tessellate once, matrices forever.
Ch 67: LINEWORK. surface_linework: boundary (domain edges, near-black) + interior iso lines (¼ ½ ¾
