# 63 Snapping — drawing becomes precise
...
<svg viewBox="0 0 680 130" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="candidates within a pixel radius of the cursor are ranked endpoint before grid before free; the winner replaces the raw point and a marker shows it" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(430,20)">
    ...
    <text x="0" y="98" fill="#666" font-size="10">radius in PIXELS (47's rule) — zoom-independent</text>
  </g>
</svg>
...
const GRID_STEP: f64 = 1000.0;   // must match the lesson-20 grid spacing (and 69's analytic
...
/// project each candidate (47's project_to_screen) and measure against the cursor. Returns the
...
`cursor_world_point()` — the function 52's clicks and 62's `on_move` both already use — grows a
snap tail, and every tool inherits snapping. In 62's body, the pick-or-z=0 code stays untouched;
what changes is that its two exits converge on one `raw` point that snap filters. Find 62's
...
Users trust snap only when they can *see* it. One white glyph at the snap point — 62's preview
...
&vec![GlyphPoint::zeroed(); 1])`, bind group on `glyph_layout`, created beside 62's preview pair;
...
Draw it right after 62's ghost block in `clear()` — same shape, sphere pipeline:
...
In `state.rs`, the cursor-move handler (right after 62's `on_move` forwarding block) pushes the
...
`self.snap_marker.as_ref().map(|(_, k)| format!("  [{}]", k.label()))` where `set_prompt` (52)
...
- Zoom far out → snapping still engages within the same ~10 *pixels* (zoom-independent, 47's rule).
...
```
Ch 62: ghosts — tools show a future; finish makes it real.
Ch 63: PRECISION. app/snap.rs: SnapKind { Endpoint/Vertex(0) > Grid(6) } — RANK first, pixel
       ...
       ONE call site — cursor_world_point(), which 52's clicks and 62's on_move already share — so
...
`64-nurbscurve.md` — Phase 10: curved geometry. The kernel's `NurbsCurve` gets drawn (sampled to a
