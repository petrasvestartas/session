# 77 Control-point editing — F10, grab a CV, reshape
...
> the parts exist: CV glyphs (64), sub-object picking (47), the drag skeleton (58), Commands (55).
> What's genuinely new is the **update economics**: a shape edit must not re-flatten the object per
> mouse-move — with 42a's arena the live path re-tessellates *nothing* and re-uploads *only the
...
<svg viewBox="0 0 680 130" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="f10 shows control points; dragging one moves the cv in the kernel and re-evaluates only affected samples; release commits one command" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(360,20)">
    ...
    <text x="0" y="34" fill="#666" font-size="10">write_buffer into 42a's arena slot (v1: glyph preview only)</text>
...
```
src/state.rs         # F10 edit mode; CV picking (47's screen radius over CV glyphs); drag routing
...
project each CV of each selected object (64's glyph positions), nearest within 8 px wins — exactly
47's vertex resolution, aimed at CVs:
...
the **camera-facing plane through the CV** (59's ray∩plane with n = view forward) — screen-natural
...
geometry carries no placement, the row does (40) — so the point goes through
...
```rust
    ...
    /// through the row's inverse placed frame, because CVs are LOCAL (40). 4-D: read the weight,
...
**This write is 42a's payoff, and it does not exist without 38a.** The per-object address comes
...
```rust
    // gpu — the surgical write 42a's slot map makes possible:
...
Curves are even lighter — `update_object_segments` rewrites their slice of the segment table (42a's
...
On **release**: one `EditShape` Command — the 54 pattern, with 55's snapshot insight doing the heavy
...
real bookkeeping: `tess_cache.remove(guid)` (65's one sanctioned invalidation) + the rebuild —
tables + `set_scene` in v1, one arena-slot rewrite with 38a — so normals and edge tubes true up at
the final shape; `hashes` refresh (42b), and the row's world box + BVH refit (36 — the shape
...
- **F10**, select 64's curve → CVs brighten. Drag one — the curve flows live under the cursor, edge
...
- Perf HUD during a drag: with 38a, uploads = the one vertex range, frame time flat — compare
  ...
  the exact hitch 42a's arena deletes.

## Recap

```
Ch 76: Phase 12 closed.
Ch 77: CV EDITING. F10 mode: CVs pick first (47's radius over 64's glyphs); drag = camera-plane
       ...
       update_vertex_range = one write_buffer into 42a's arena slot / update_object_segments for
       curves — REQUIRES 42a's guid→slot map; v1 without it previews glyphs live and commits via
...
`78-edit-points.md` — CVs are *off* the curve (they pull from a distance); **edit points** are *on*
