# 83 Section planes — cut the building open
...
<svg viewBox="0 0 680 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="a section plane cuts the model; fragments on the negative side are discarded per pixel; the plane is set by three points or the work plane and dragged along its normal" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(330,24)">
    ...
    <text x="0" y="58" fill="#d7dae0">set: 3 points · work plane (80)</text>
    <text x="0" y="78" fill="#d7dae0">move: drag = 59's axis math</text>
...
> field is an **E0063** build error — same as 80's `work_plane`). Step 4's `state.sections…` code needs it.
...
```rust
            wgpu::BindGroupLayoutEntry {
                binding: 1,                                    // sections (83) — every shader sees it
...
`section` composes machinery you already have — 54's multi-step points, 80's work plane, 59's axis
drag:
...
```rust
        "section" => match parts.next() {
            ...
            _ => { let (cmd, get) = SectionBy3Points::start();   // 80's 3-point flow, verbatim
                   Dispatch::Start(cmd, get) }
        }
```

`SectionBy3Points` is 80's `Cplane3Points` **verbatim** except its last two lines — copy that
...
The drag is 59's skeleton pointed at a plane instead of an object — live `poke()` per move,
...
(Both commands live in `commands.rs` beside 80's; `state.cursor_ray()` goes `pub(crate)` so they
can call it — one-word edit in `state.rs` (59). `Plane::from_point_normal(point, normal)` and
...
```
Ch 82: the capstone loop.
Ch 83: SECTIONS. Up to 4 world planes in one uniform, rebased camera-relative per frame (d_rel =
       ...
       never disagree. Command: `section` = 3 points (54), `wp` (80), off/flip; `section drag` =
       59's closest_param_on_axis aimed at the plane's own normal, live per move. Sections are
...
`84-import-export.md` — the kernel's OBJ codec (and one honest gap about STEP) reaches the browser:
