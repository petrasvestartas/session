# 50 Hidden objects — visibility is real state
...
> respected by **drawing** (41's shader collapse), **picking** (46/48), and **marquee** (49) — three
...
```
src/engine/gpu/mod.rs   # set_flag_rows(flag, rows) — the generalization of 49's set_selected_rows
...
49's `set_selected_rows` and the visibility upload are the same function with a different bit. Extract
it — this is the moment the pattern earns a name (49's tables-then-live-row discipline comes along:
...
```rust
    ...
    /// tables first (the truth), then the live instance via write_row_flags (49). One function
    /// serves SELECTED (49) and HIDDEN (here) — CULLED (41) keeps its own per-frame path.
...
and 49's function becomes a wrapper (find `set_selected_rows`, replace its body):
...
Same caveat as 49's `selected`: guids can collide across docs, so a hide keyed by guid can catch a
...
```rust
    // pick_mesh (46) — first line inside `for (row, guid) in cands {`  (guid is an owned String → &guid)
    if self.hidden.contains(&guid) { continue; }

    // pick_thin (48) — before the `match` inside `for h in hits {`  (h.guid() already returns &str)
    if self.hidden.contains(h.guid()) { continue; }

    // select_marquee (49) — first line inside `for row in 0..self.world_boxes.len() {`
...
```
Ch 49: selection state + marquee.
Ch 50: VISIBILITY. Scene.hidden (parked since 35) becomes runtime state with three consumers that
       ...
       49's flip-tracked upload — tables.objects[row].2 is the truth (hides survive later Msg::File
       ...
       clears. H / Ctrl+H until the CLI takes over (52). Phase 7 complete: ray → hit →
...
`51-egui-hud.md` — Phase 8: the interface. An egui overlay lands on top of the 3-D pass — the perf HUD
