...
calls** the frame actually issued. Console-first today (graduates to an on-screen HUD in lesson 62),
...
Ch 28: add the gauge first. A Performance struct times each frame (exponential-average ms → fps) and takes
       ...
       measured, not assumed. Console now; on-screen HUD in lesson 62.
```
