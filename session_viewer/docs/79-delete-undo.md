# 79 Delete + undo/redo — the pattern everything stands on
...
> and *every* later mutation (gumball drags 67, drawing tools 70, CV edits 85) will be born as one of
...
holds the object's vertices; the instance flag makes every lane skip them (59's shader collapse).
True row/arena reclamation is 48's business — when its free-list lands, `RemoveObjects` upgrades
...
impl History {
    ...
    /// 49's reconcile) — call this there, or undo would resurrect the WRONG object.
...
impl RemoveObjects {
    /// Snapshot NOW, at construction — before anything mutates. (58's selection is row-keyed,
...
Three new arms in `dispatch` (add `"delete"`, `"undo"`, `"redo"` to 63's `VERBS`; aliases
...
- **An `Rc` snapshot PINS its geometry.** A deleted object's mesh stays fully alive as long as its
  ...
  reclaim — 45); the pin is the *kernel-side* copy, and it shrinks only when entries fall off.
- **Coalescing: not in v1, and here's the line.** A gumball drag is already ONE command (67:
  snapshot at press, commit at release — the hundred intermediate live frames never touch the
  stacks). What floods a stack is micro-commits — 69's typed nudges, arrow-key moves: 200 nudges =
...
- **Clear on scene rebuild.** Snapshots are row-keyed, and rows are positional: a fresh
  `set_scene` or 49's reconcile reorders them, after which "restore row 17" would resurrect the
  WRONG object or flip another row's flags. Call `history.clear()` wherever a scene rebuild lands —
  undo across a rebuild is meaningless anyway. (58's `selected` is row-keyed too and wants the
  same remap-or-clear treatment there.)

## Step 4b — caches leave with their objects

The geometry block's caches are keyed by guid and MUST be evicted on remove, or a long
session leaks meshes for dead objects and a stale pick can hit a ghost. In `RemoveObjects`'
apply path, per removed guid (cheap no-ops for other types):

```rust
        scene.curve_cache.remove(&guid);   // 43's samples
        scene.tess_cache.remove(&guid);    // 44/46/47's tessellations (+ linework)
```
...
- Select two objects → **Del** → gone; log `delete 2 object(s)`; the HUD object count does NOT
  move — v1 delete *hides* the rows, it doesn't reclaim them (59's collapse, 48's future free
  list), so "deleted" costs memory but zero draw time.
- **Ctrl+Z** → both return, same colors, same PLACEMENT (move an object first with 67's gumball if
...
```
Ch 63: history/autocomplete — CLI ergonomics.
Ch 64: UNDO. trait Command { apply / revert / label } + History { done, undone } — the archive's
       ...
       Delete v1 hides, 45 reclaims. delete/undo/redo verbs; Del / Ctrl+Z / Ctrl+Y just type them.
...
`80-gumball-geometry.md` — Phase 9: transform & draw. The 3-axis gizmo appears at the selection
