...
- Beside `#viewer-status` (`index.html:54`); it and `#viewer-error` (`:51`) are markup Rust only *finds* (`feedback.rs:15`, `:28`). Nothing under `src/` creates an element; one would put layout in a string literal.
...
- Overlay the canvas, never shrink it (`100vw`/`100dvh`, `index.html:28`): shrinking fires `State::resize` (`src/state.rs:212`) per show/hide, re-uploading controls and re-targeting every lane.
...
- So `Input::key` (`src/app/input.rs:51-86`) needs no guard, boolean or mode enum: each is a weaker copy of the browser's decision.
...
- Shaped on `PointerCancellation` (`src/app/input.rs:222-256`): own element and closure, detach in `Drop` (`input.rs:249-255`), so JavaScript cannot hold a handle into freed wasm memory.
- Install in `App::resumed` beside the pointer listener (`src/lib.rs:128-132`), another `proxy.clone()`.
- Two `Msg` variants (`src/lib.rs:36-50`), two `user_event` arms (`:144-185`):
...
- The picture follows by itself: `user_event` ends in `request_if_needed()` (`src/lib.rs:186`), every named action in `State::touch()` (`src/state.rs:242`).
- `State` lives inside the event loop (`src/lib.rs:143`), so a DOM callback's one path to it is the `EventLoopProxy` (`loader::post`).
- Not a `#[wasm_bindgen]` export: `reload_scene` is one because the embedding page calls it (`index.html:76`); the inline script (`:60-86`) stays lifetime wiring.
...
- Validate sign and finiteness in the arm: `Session::add_brep` refuses only zero faces *and* zero vertices (`session_rust/src/session.rs:1454-1463`), so a negative radius sails through.
- Report the kernel's refusals: `add_polyline` `None` below 2 points (`session.rs:1379`), `add_nurbscurve` below 2 CVs (`:1422`), `add_mesh` when empty or faceless (`:1411`).
...
|---|---|---|
| `box` | `[sx [sy sz]]` | `BRep::create_box` (`brep.rs:588`) then `Session::add_brep` (`session.rs:1454`) |
| `cyl` | `[r h]` | `BRep::create_cylinder` (`brep.rs:604`) |
| `sphere` | `[r]` | `BRep::create_sphere` (`brep.rs:633`) |
| `cone` | `[r h]` | `BRep::create_cone` (`brep.rs:667`) |
| `pyramid` | `[base h]` | `BRep::create_pyramid` (`brep.rs:693`) |
| `torus` | `[R r]` | `BRep::create_torus` (`brep.rs:749`) |
...
| Command | Calls |
|---|---|
| `fit` | `State::fit_selected_or_all` (`src/state.rs:191`) |
| `hide` | `State::hide_selected` (`src/state.rs:280`) |
...
| `esc` | `State::escape_selection` (`src/state.rs:567`) |
| `help` | `CMDS` itself |
...
- In `src/camera.rs` beside `view_proj_anchored` (`:219`): it inverts a matrix built there, and elsewhere duplicates the reverse-Z and unit conventions.
- Unit trap: `view_proj_anchored` works in metres (`self.unit.to_meters()`, `src/camera.rs:219-224`), world positions in millimetres. Divide back, or the ray is off by a thousand and the hit lands past the far plane.
- `Xform::inverse()` returns `Option<Xform>` (`session_rust/src/xform.rs:390`), so the signature is `Option` out: a degenerate projection is real right after a resize to zero.
- The hit itself is kernel math: a `Line` from the ray through `intersection::line_plane(&line, plane, false)` (`session_rust/src/intersection.rs:198`), the plane's projection of the ray origin as the parallel fallback.
- Do not port the archive's rule for *choosing* the plane (`ProjMode` plus a view-matrix column); re-derive from `set_view` (`src/camera.rs:273`) and `toggle_projection_framed` (`:154`).

## Snap is CPU work, and picking is not

- The pick path answers *which object*: async, once per click, halo, generation guard (`src/state.rs:498-525`, applied atop `render`, `:417`). Tools needing identity use it untouched.
...
- Two fields on `Gpu` (`src/engine/gpu/mod.rs:59-89`), after `controls` and `control_net`:
...
- Beside `status` in `src/app/feedback.rs`. `set_text_content`, never `set_inner_html`: the module doc states it (`feedback.rs:1`), both writers follow it (`:17`, `:30`), and a log echoing typed text is where the rule is load-bearing.
- `status` stays a one-line transient: "Select one object before pressing F10" (`src/state.rs:530`).
...
- Label the transaction with the typed line: the label lives on the `Transaction` (`session_rust/src/history.rs:161-174`), so a history listing reads back as a command log for free.
...
- The document holds `Rc<Session>` (`src/app/scene.rs:30`) and every mutator takes `&mut self`: the split is `Rc::make_mut`.
- Why: one manifest listing a file twice hands both documents the same `Rc`, so without the split, moving one placement moves the other and the live source's cached copy (`an_edit_must_split_a_session_two_placements_share`; `FileDoc` comment, `:26-37`).
- The deep copy costs only when the `Rc` is shared; a loader-made document is uniquely owned.
- Refuse an edit on a live document rather than split it: `LiveSource` keeps its own `Rc<Session>` per URL (`src/app/live.rs:102`, `:352`, `:395`), so the count is at least two and the poller would overwrite the edit on its next tick. Say so on the log line.
...
- Beside `hidden_rows` in `src/app/scene.rs`. `None` for streamed clouds, sheets and text rows, as `Scene::geometry` already is (`:525-531`): `add_streamed_cloud` and `add_sheet` push a `FileDoc` with an empty `Session::new(&name)` shell, `display_only: true` (`:355-361`, `430-436`).
- Refuse a whole-scene rebuild while `scene.streamed` or `scene.sheets` is non-empty — "Streamed clouds and sheets cannot come back (no kernel object)" (`:192`, warnings at `:199-205`) — or an edit elsewhere silently discards a multi-million-point resident prefix.
- A sheet is one row for tens of thousands of segments (`push_row` once, `:416`), so deleting the selection deletes the whole sheet; the entity is metadata (`SheetBatch.resolved`, `:89`), not editable geometry.

### Keeping the selection across a rebuild

- `reset_rows` sets `selected = None` and clears `guid_to_row` (`src/app/scene.rs:178-189`), so a rebuild destroys every row number.
- Capture `Scene::identity_of(row)` first (`:596`), re-resolve after through a new `Scene::row_of(&identity)` over the private `guid_to_row`.
- The hide set is the model: it survives because it stores `(document index, guid)` (`:129`), and `add_file` re-reads it into `FLAG_HIDDEN` as rows return (`:271-278`).
...
- A pure translate updates `translation[row]` in f64, re-anchors that one value against `last_origin`, writes one `[f32; 4]`: **16 bytes**. Rotate or scale also rewrites `rows[row].model` with columns 12/13/14 zeroed and writes one `Instance`, **96 bytes** by the const assert at `src/engine/gpu/instance.rs:23` — **112 bytes**, seven times a translate.
...
- `coords::parse`, `lookup` and the argument helpers are pure and test natively, with in-file `#[cfg(test)] mod tests` as `src/app/scene.rs:635` and `src/engine/gpu/objects.rs:496` do.
- `State::command` cannot: `State::new` takes an `Arc<Window>` and is built only in the wasm loader (`src/app/loader.rs:120`), while the native harness builds `Scene`, `Gpu`, `Camera` directly (`src/selftest/lifecycle.rs`).
...
2. **Dispatch over actions that exist.** `mod command;` (`src/state.rs:18-20`), a `shell` field (`:36-59`) initialised in `State::new` (`:68-87`), arms calling `fit_selected_or_all`, `hide_selected`, `show_all`, `clear`, `toggle_xray`, `enable_controls`, `escape_selection`, `set_cloud_size`; results to `self.status()`.
3. **The log sink and the markup.** `feedback::log`, the three elements, `State::command` writing there.
4. **The field is live.** Three web-sys features, `CommandLine`, `Msg::Command`/`CommandCancel`, installed in `App::resumed`. Ten commands end to end; `f` in the field no longer fits the view, free from `src/lib.rs:212`.
