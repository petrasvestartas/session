...
<svg viewBox="0 0 680 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="the docs each contribute a top-level row; each session tree flattens to visible rows only; each row shows an eye toggle and a name; toggles call the same scene verbs as the CLI" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(280,20)">
    ...
    <text x="0" y="54" fill="#888">eye → scene.hidden + flags (51's verbs — ONE authority)</text>
    <text x="0" y="74" fill="#888">click → scene.selected (50's verbs)</text>
...
```rust
    ...
    /// Renderable guids of doc `d` with no tree node — collection-only citizens (69) become
...
`remove_object` maintains (56), so deleted objects leave the panel automatically.)
...
`TreeIntent { toggled, expand_toggled, clicked }` collects inside the closure (52's rule); `State`
...
- Click a name → viewport selection + gumball follow (50/57). Shift+click adds. The selected row
  reads black-on-white.
- Delete an object (56) → its row is gone next frame; undo → row returns. The panel was never told —
...
`76-tree-viewport.md` — the two views converge: picking in the viewport reveals and scrolls the tree
