# 90 GPU trimming — the CDT stays home, the fragment decides
...
> polygonizing the trim region entirely**: tessellate the FULL UV rectangle with 74's dumb
...
ship. The GPU consumes *polygons*, never pcurves: all NURBS math stays in one place (73's
...
One invocation per CELL of 74's grid, once per (re)tessellation:
...
is 8M cheap tests, once per re-tessellation, in a dispatch that runs beside 74's. Never per
frame.

## Step 3 — outside cells die in compute, boundary cells carry a flag

Extend 74's `tess` entry (or a small follow-up pass): after writing the vertex, each OUT
...
47's test set is the acceptance: a surface with a **circular hole** and one with a **concave
outer boundary**. Side by side against the CPU build (env-flag the old path), the silhouettes
match to sub-pixel; the interior shading is identical (same 74 vertices). Then the perf claim:
...
```
Ch 75: the CDT does not port (sequential by construction) and does not need to: full-rect
...
`91-gpu-brep.md` — faces are trimmed surfaces, edges are curves, and the CPU mesher's hardest
