# 68 Ray-cast meshes — nearest hit wins
    ...
    pub fn pick_ray(&mut self, ray: &Ray) -> Option<crate::app::pick::PickHit> {
        ...
        for (row, guid) in cands {
            ...
            let hit = match self.docs[d].session.lookup.get_mut(&guid) {
                ...
                _ => None,   // Line/Polyline/Point/PointCloud → lesson 70 (thin geometry needs a
...
lesson 70 with the other thin geometry.
...
`69-subobject-picking.md` — a hit tells you *which mesh*; sub-object picking tells you *which part*. From
