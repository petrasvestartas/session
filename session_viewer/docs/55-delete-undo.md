# 55 Delete + undo/redo — the pattern everything stands on
...
holds the object's vertices; the instance flag makes every lane skip them (50's shader collapse).
True row/arena reclamation is 42's business — when its free-list lands, `RemoveObjects` upgrades
...
Three new arms in `dispatch` (add `"delete"`, `"undo"`, `"redo"` to 54's `VERBS`; aliases
...
`match event.logical_key.as_ref()`, beside the Escape arm (52):
...
- **Ctrl+Z** → both return, same colors, same PLACEMENT (move an object first with 58's gumball if
...
```
Ch 54: history/autocomplete — CLI ergonomics.
Ch 55: UNDO. trait Command { apply / revert / label } + History { done, undone } — the archive's
...
`56-gumball-geometry.md` — Phase 9: transform & draw. The 3-axis gizmo appears at the selection
