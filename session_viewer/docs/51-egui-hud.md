# 51 egui overlay — the HUD and first settings
...
(`drawn`/`total` come from 41's `perf_drawn`/`perf_total` on `Gpu`.)
...
```rust
        // 1. feed the HUD (the Step-1 accessors + 41's cull counters)
...
```
Ch 50: visibility — Phase 7 closed; the scene is fully interactive but mute.
Ch 51: EGUI OVERLAY. Shell { ctx, winit-state, wgpu-renderer } (archive wiring:
...
`52-command-bus.md` — THE interface arrives: a command line docked at the screen edge, a registry of
