...
pub struct CommandKeys {
    input: web_sys::HtmlInputElement, // the command box
    callback: wasm_bindgen::closure::Closure<dyn FnMut(web_sys::KeyboardEvent)>, // the key handler
    blur: wasm_bindgen::closure::Closure<dyn FnMut(web_sys::Event)>, // closes the box on focus loss
}

#[cfg(target_arch = "wasm32")]
impl CommandKeys {
    /// Install once: Enter sends the line, Escape discards it.
    pub fn new(
        input: web_sys::HtmlInputElement, // the command box
        proxy: winit::event_loop::EventLoopProxy<crate::Msg>,
    ) -> Result<Self, wasm_bindgen::JsValue> {
...
/// One click listener for the whole layers panel.
#[cfg(target_arch = "wasm32")]
pub struct LayerClicks {
    panel: web_sys::Element, // the layers panel
    callback: wasm_bindgen::closure::Closure<dyn FnMut(web_sys::Event)>, // the click handler
}
