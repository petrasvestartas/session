...
impl State {
    ...
    pub fn end_gizmo(&mut self, x: f64, y: f64) -> bool {
        ...
        // the delta is world, the session stores local
        let Some(local) = self
            .scene
    ...
    /// Undo can add or remove objects, so rows are rebuilt.
...
/// Segments in a quarter of a rotation arc.
const ARC_STEPS: u32 = 12;

/// The two axes a rotation arc about `axis` is drawn in.
...
/// The three axis colours, packed for the lanes.
...
/// The widget's rows: arms and arcs as strokes, balls as markers.
fn widget_rows(origin: &Point, per_px: f64, pixel_scale: f64, widget: u32) -> (SegRows, GlyphRows) {
    ...
    // the three rotation arcs, where `Gizmo::hit` looks for them
    for (i, axis) in [Axis::X, Axis::Y, Axis::Z].into_iter().enumerate() {
...
/// A packed row colour as four floats.
...
mod tests {
    ...
    fn the_widget_draws_three_arms_three_arcs_and_four_balls() {
        ...
        // an arm and its ball share one colour
    ...
    fn the_widget_reaches_the_pixels() {
        ...
        // 72 CSS px of arm fits inside a 200-unit view
