...
/// The three axis colours, packed low byte red.
...
/// The widget's rows: arms and arcs, balls and hub.
fn widget_rows(origin: &Point, per_px: f64, pixel_scale: f64, widget: u32) -> (SegRows, GlyphRows) {
    ...
    let stroke = |p0: [f64; 3], p1: [f64; 3], color: u32| CylinderSegment {
        p0: render_position(p0), // arm start
        p1: render_position(p1), // arm end
        radius: 0.0,
        color,
        instance_id: widget, // the gizmo's own row
    ...
    for (i, axis) in [Axis::X, Axis::Y, Axis::Z].into_iter().enumerate() {
        ...
        glyphs.dots.push(GlyphPoint {
            center: render_position(at(ball)), // the ball position
    ...
    // three rotation arcs, one quarter circle each
    for (i, axis) in [Axis::X, Axis::Y, Axis::Z].into_iter().enumerate() {
...
/// A packed row colour as four floats, low byte red.
...
mod tests {
    ...
    fn the_widget_draws_three_arms_three_arcs_and_four_balls() {
        ...
        // arm and ball share one packed colour
    ...
    fn the_arcs_are_where_the_hit_test_expects_them() {
        ...
        // the Z arc lies in the x/y plane
