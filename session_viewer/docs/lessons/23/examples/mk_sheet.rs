//! Publish a session as one sheet: flat stroke arrays plus a .meta side table.
...
const CHORD_DEGREES: f64 = 5.0; // curve turn per sample
...
/// Control point `i` in plain coordinates.
...
/// Total turn of the control polygon, in degrees.
...
/// Sample the curve; more points where it turns more.
...
impl Sheet {
    /// Add one stroke and its meta row.
...
/// `mk_sheet <in.pb> <out>` writes `<out>` and `<out>.meta`.
fn main() {
...
/// The geometry kind as a short name.
