...
fn instance_layout(device: &wgpu::Device) -> wgpu::BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        ...
        entries: &[storage_entry(0), storage_entry(1)], // rows and translations
    })
}
...
impl Layouts {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        Self {
            mvp: uniform_layout(device, "mvp.layout", wgpu::ShaderStages::VERTEX_FRAGMENT), // the camera matrix
            line: uniform_layout(device, "line.layout", wgpu::ShaderStages::VERTEX_FRAGMENT), // pen settings
