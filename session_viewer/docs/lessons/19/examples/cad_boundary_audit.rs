//! Audit CAD edge chains against the triangle edges of each face mesh.
...
/// A position as exact bits, -0.0 folded to 0.0.
...
/// An edge key that ignores direction.
...
/// The f64 position of mesh vertex `key`.
fn at(mesh: &Mesh, key: usize) -> [f64; 3] {
    let vertex = &mesh.vertex[&key];
    [vertex.x, vertex.y, vertex.z]
}

/// The position rounded the way the GPU upload rounds it.
...
/// Every source triangle edge of the mesh.
...
/// Every triangle edge of the rendered mesh.
...
/// Whether two chains are the same polygon, either direction.
...
/// Whether a seam use matches the owner chain.
...
/// Write primitive fixtures, then audit the given .pb files.
fn main() {
