# 68 Trimmed surfaces — first-class, and the every-map rule becomes structure
...
- **`add_file` (the walk):** one `match ObjRef` — `Geom` keeps the existing inner match (which
  already has 60/65's curve and surface arms); `Trimmed` goes through the cache.
...
- **pick:** `Trimmed` resolves through `render_mesh` (67), exactly like surfaces already do.
...
```rust
    // render_mesh (67) gains the source arm — trimmeds live OUTSIDE lookup, so chain a second
    // doc scan after 67's lookup match:
...
```rust
/// Trim boundary loops as tubes: sample each UV loop (64's sampler), LIFT (u,v) → 3D, tube.
/// Destined for tables.pipes (SOLID lane, 66's rule — flat segments would z-fight the skin);
...
fn trimmed_linework(ts: &NurbsSurfaceTrimmed) -> Vec<CylinderSegment> {
    ...
    let mut one_loop = |uv: &NurbsCurve| {
        ...
        for q in sample_curve(uv) {                       // q = (u, v, 0) in the param square
            ...
            if let Some(a) = &prev {
                if a.distance(&p, None) > 1e-9 {          // 67's zero-length filter
...
cache entry — a one-word edit to 61/67's arm. One honest caveat: there is no
...
```
Ch 67: BRep — debts paid.
Ch 68: TRIMMED + STRUCTURE. NurbsSurfaceTrimmed::mesh() (= mesh_q(20°, 0.005), holes honored in the
       ...
       trim-blind); transforms already free (58's type-blind apply_world_delta), reconcile
...
`69-ground-grid.md` — Phase 11: rendering quality, engineered fast. First the stage: an analytic
