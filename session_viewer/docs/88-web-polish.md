# 88 Web polish — load progress and a shippable wasm
...
```
Ch 87: the workflow.
Ch 88: THE LAST MILE. Streamed fetch: ReadableStreamDefaultReader chunks + Content-Length →
...
`89-textures.md` — an **optional appendix** outside the CAD default look: upload an image, bind a
...
files: `_KERNEL_GAPS.md` (the STEP port is the big one) and 80's perf levers, both waiting on real
