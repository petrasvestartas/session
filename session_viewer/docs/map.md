...
A compressed copy stays **pinned at the top of the page**, following you from step to step and from one code block to the next inside a step.
...
Two zones lit: the change crosses a boundary, and those are the steps where a file is written across several steps and the build goes red in between. The **Does it compile yet?** line at the top of each lesson names them.
