# 81 Capstone — the full loop, on real work
...
<svg viewBox="0 0 680 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="the acceptance loop: load, fit, pick, tree reveal, gumball, numeric entry, draw with snap, save, undo history — run on both the floor model and the pdf drawing" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g fill="#d7dae0" font-size="10" text-anchor="middle">
    <text x="56" y="37">load .pb (34)</text><text x="162" y="37">fit (15)</text><text x="268" y="37">pick (42–44)</text><text x="374" y="37">tree reveal (75)</text>
    <text x="56" y="83">gumball (52–56)</text><text x="162" y="83">draw + snap (57–59)</text><text x="268" y="83">save (43)</text><text x="374" y="83">undo walk (55)</text>
  </g>
...
1. **Load + fit.** 491 objects log (34a); `F` frames the floor (34b). HUD: 60 fps, single-digit
   draws, `drawn/s = 0` once you let go (70).
2. **Pick the far corner beam.** Click it → guid logs, outline rings it (73), gumball at its volume
   center (56), tree unfolds to its row (75). Shift+click a second beam — both ring, centroid moves.
3. **Gumball.** Drag X: live, axis-locked (58). Click the arrow, type `250` ⏎ — exact (60). Rotate
   the arc 30° — pinned to the plane (59).
4. **Draw with snap.** `cplane` onto a beam's face (79). `rect` two corners with `[End]` snaps (63).
   `box`, height `120`. `curve` through four clicks (64). Every one lands *exactly* — `probe`
   endpoints to prove it (53).
5. **Save.** Wait the debounce (43) → one download. Policy: save the ACTIVE doc — this script edits
   one sheet; an all-docs save is the same call in a loop over `scene.docs`. Reload the saved file
   (point a manifest item at it, or 44's watch): your box, rect, and curve are there, placed to the
   digit.
6. **The undo walk.** `undo` repeatedly, all the way down: curve → box → rect → rotate → 250-move →
   drag — each reverts *exactly*, in reverse birth order, mixed kinds (55's promise). `redo` back to
   the top. The scene must land byte-identical — hash the session (42b's fingerprints) before and
...
1. Load, `F`, orbit — interactive (31's one draw + 41's cull). `drawn/total` moves as you zoom.
2. **Pick one line in dense hatching** — the intended one wins (48's tolerance + priority).
   **Marquee a few thousand** — instant, exact (49).
3. **Gumball-drag the marquee'd region** — live at full rate (58's matrix-only; the stress gate).
   Numeric-move it back `−(what you dragged)`; verify with `probe` it's home.
4. Hide the selection (50), marquee again — hidden lines neither pick nor select. Show all.
...
|---|---|
| picks the wrong thing / nothing at density | 48's tolerance, 46's BVH candidates, 40's world boxes |
| a moved object saves in its old place | the move bypassed 58's `apply_world_delta` (Session xform never written), or 38b/43's fingerprint missing `session.xform` |
| undo restores at the wrong row / loses placement | 51 — snapshot missing its `local` xform or doc; 38b `commit` renumbering |
| tree and viewport disagree | a mutation bypassing the Scene verbs (45/50's single authority) |
| fps sags on scene B only | a per-object CPU loop that scales with N — profile before pulling 80's levers |
| something draws but won't pick / hide | a map missed a source — `all_objects()` audit (68) |
...
Phase 6    (38–40): the file is ALIVE — per-object arena; reconcile is per-DOC, diff by content
                    hash (which includes session.xform — 42b's rewrite), save gates, watch.
...
Capstone   (81):    the loop, twice, green.
```
...
Where next: Phase 14 (78–84 — sections through web polish) and the textures appendix (89), 80's
