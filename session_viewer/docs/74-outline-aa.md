# 74 Selection outline + AA polish — the look, without the tax
...
> the mask renders **only when dirty** (71's flag, scoped), the search becomes **separable** (two
...
<svg viewBox="0 0 680 140" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="selected objects render once into a small mask when selection changes; a horizontal then vertical distance pass expands it; composite draws a crisp ramp where distance is under the width" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <text x="340" y="112" fill="#888" text-anchor="middle">nothing selected → all three passes skip; static scene → nothing renders at all (71)</text>
...
72's AO texture; reuse the fullscreen `samp` sampler the pass already binds), plus the ring width:

```wgsl
// find the composite binding block (72's AO texture) → add after it:
...
The plumbing is 72's post-process pattern, one more time. **Targets** — beside 72's half-res AO pair
...
frame entry point, between 72's blur pass and the composite (the composite samples
...
built once in `Gpu::new`, the same `build_*_pipeline` + layout/bind-group moves as 72's gtao/blur
...
- Select an object → a crisp amber ring, ~3 px, smooth on diagonals (coverage + ramp), hugging the
  silhouette — including through holes (69's trimmed surface outlines its hole rim too: the mask is
...
`75-scene-tree.md` — Phase 12: scene management UI. The Session's tree appears in a panel —
virtualized so a thousand rows scroll smoothly, with an eye icon driving 51's visibility per object.
