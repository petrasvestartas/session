...
- **texture** — a grid of texels with a format: the surface's colour format; `Depth32Float` (04a, one depth per pixel); `Rg16Uint` (05, the triangle id target the ink visibility test reads); `Rg32Uint` (12, pick ids); `R8Unorm` (11, glyph coverage; 12, the selection coverage mask; 17, the silhouette masks).
