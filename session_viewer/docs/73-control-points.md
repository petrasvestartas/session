...
> mouse-move — with 38a's arena the live path re-tessellates *nothing* and re-uploads *only the
...
<svg viewBox="0 0 680 130" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="f10 shows control points; dragging one moves the cv in the kernel and re-evaluates only affected samples; release commits one command" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(360,20)">
    ...
    <text x="0" y="34" fill="#666" font-size="10">write_buffer into 38a's arena slot (v1: glyph preview only)</text>
...
geometry carries no placement, the row does (36) — so the point goes through
...
```rust
    ...
    /// through the row's inverse placed frame, because CVs are LOCAL (36). 4-D: read the weight,
...
**This write is 38a's payoff, and it does not exist without 38a.** The per-object address comes
...
On **release**: one `EditShape` Command — the 54 pattern, with 51's snapshot insight doing the heavy
...
real bookkeeping: `tess_cache.remove(guid)` (61's one sanctioned invalidation) + the rebuild —
tables + `set_scene` in v1, one arena-slot rewrite with 38a — so normals and edge tubes true up at
the final shape; `hashes` refresh (38b), and the row's world box + BVH refit (36 — the shape
...
- Perf HUD during a drag: with 38a, uploads = the one vertex range, frame time flat — compare
  ...
  the exact hitch 38a's arena deletes.
...
Ch 73: CV EDITING. F10 mode: CVs pick first (43's radius over 60's glyphs); drag = camera-plane
       ...
       update_vertex_range = one write_buffer into 38a's arena slot / update_object_segments for
       curves — REQUIRES 38a's guid→slot map; v1 without it previews glyphs live and commits via
