...
```wgsl
    ...
    var ambient = scene_rgb * sky_vis;                          // scene_rgb = 72's lit color; there is no separate ambient term
```
...
`74-outline-aa.md` — the last archive-look piece: the crisp selection outline, rebuilt without the
per-frame tax — mask passes render only when dirty (71), the 81-tap box search becomes two separable
