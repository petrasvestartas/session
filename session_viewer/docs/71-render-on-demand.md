# 71 Render-on-demand — the biggest win never touches the image
...
| selection & hover changes | 50's gestures, 58's gumball hover |
| any Command executed / undone / redone | `commit()`, the undo/redo verbs (56) |
| live gumball drag | `set_live_model` callers (59) |
| ghost preview updates | `set_preview` / `clear_preview` callers (63) |
| reconcile applied (watch/reload) | `apply_session` (45) |
| settings toggles, thickness slider | the apply-intent block (52) |
...
```rust
    pub fn render(&mut self) -> anyhow::Result<()> {
        ...
        // …the entire existing frame: cull (41), UI build (52), gpu.clear(…) — unchanged…
...
(Sweep the table's rows to call `poke()`. The watch poll (45) and 44's debounce still need ticks
...
1. `UiState` (52) gains `pub frames_drawn_last_sec: u32,` (seed `0` in `State::new`'s `UiState`
...
```rust
        ...
        self.ui.frames_drawn_last_sec = self.frames_drawn_last_sec;   // feed the HUD, 52's step 1
```

3. In `build_ui` (52), find the perf window's first label —
...
Ch 71: RENDER-ON-DEMAND. Games render time; CAD renders STATE. One dirty flag; poke() = dirty +
       ...
       0 idle, display-rate while interacting. Static scenes now cost zero GPU; 72's AO rides on
...
`72-gtao.md` — ambient occlusion, the constant-quality way: half-resolution GTAO with a fixed tap
