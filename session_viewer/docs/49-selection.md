# 49 Selection — highlight + marquee

> **Big picture.** *Phase 7.* Picking (42–44) answers a click; **selection is state** — the set of
> objects every later tool acts on: gumball (56) moves the selection, `delete` (55) removes it, the
> tree (74) mirrors it. This lesson stores that set, makes it *visible* (a tint via one instance-flag
...
```rust
    ...
    /// crop · view_proj remaps the rect to the full clip cube, so Gribb–Hartmann (41) needs
    /// no change.
...
(Convert cursor px → NDC with 45's two formulas; make sure `x0 < x1`, `y0 < y1` after sorting the
...
Press and release split in **lib.rs** — replace 45's Left-button `MouseInput` arm with:
...
and in `state.rs`, rename 41–48's `on_left_click` to `on_left_release`. Its body keeps the
...
```rust
        // release: `self.cursor` is where we let go (45's px field); press stashed self.drag_start.
        ...
        if drag < 3.0 {
            // CLICK: replace (or Shift → toggle). `ray`/`tol` as built at 42/48's pick site.
            ...
            // MARQUEE (Shift → additive). px → NDC (45's two formulas), then sort so x0<x1, y0<y1.
...
visual to 51's egui, which gives it for free; selection is correct either way.)
...
```
Ch 48: thin picking — radius + mesh-wins-ties.
Ch 49: SELECTION. State = Scene.selected: HashSet<guid> — the set every later tool acts on. Visible
...
`50-hidden-filter.md` — visibility becomes real state. 35 parked a `hidden` set and a flag bit; now
`hide`/`show` actually work end to end: a hidden object doesn't draw (already), doesn't pick, doesn't
marquee — and the little state machine here is the groundwork for the first CLI verbs (52).
