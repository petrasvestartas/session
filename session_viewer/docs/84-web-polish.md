...
```rust
    ...
    let bytes = fetch_bytes_with_progress(url, move |loaded, total| {
        ...
        if pct >= last.get() + 5 || pct == 100 {
            last.set(pct);
            push_status(format!("loading… {pct}%  ({loaded} bytes)"));
        }
    }).await.unwrap_or_default();
```

(`push_status` = the same static-queue-to-CLI-log channel 83 built for GPU errors — second
customer. Note the read loop yields to the browser between chunks — the tab stays live, which *is*
...
Nothing is queued after this — the roadmap's 84 lessons are written. What remains is the honest
backlog, in two files: `_KERNEL_GAPS.md` (the STEP port is the big one) and 76's perf levers, both
waiting on real scenes to demand them. Build something with it.
