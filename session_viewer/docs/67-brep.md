...
> on every use** (34's noted caveat, 46's pick does it *per ray*), its **edges** draw as tessellation
> wireframe instead of its real curve network, and its transform path re-flattens. This lesson pays
> all three with machinery that now exists: 65's cache, 66's linework split, 58's matrix-only commit.
...
<svg viewBox="0 0 680 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="a brep is faces plus shared edge curves as one object; the viewer caches one tessellated mesh and draws real edge curves; picking any face selects the whole brep" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(300,20)">
    ...
    <text x="0" y="54" fill="#666" font-size="10">edges = m_curves_3d sampled → 31's tubes (66's recipe)</text>
    <text x="0" y="70" fill="#666" font-size="10">transform = apply_world_delta (58) — set_xform, nothing bakes</text>
...
so 64's "every map" work was done back in 34–38; what changes is *how well* each arm treats it.)

## Step 1 — cache the tessellation: `src/app/scene.rs`

Every `b.mesh()` call re-runs the tessellation pipeline. Route BReps through 65's cache — same map,
keyed by guid, filled on first touch:

```rust
    // 65's surface_mesh generalizes: rename to render_mesh(guid) and add the BRep source —
...
`add_file`'s BRep arm, the pick arm (46's `pick_mesh` — this one was re-tessellating **per
...
priming pass at the top of `add_file`, 66's warm/read sites) to `render_mesh` — same signature, 62
...
with 64's curve recipe instead of showing tessellation wireframe:
...
/// instance_id stays 0 in the cache — stamped at push time (66's rule).
fn brep_linework(b: &BRep) -> Vec<CylinderSegment> {
    let mut segs = Vec::new();
    let edge = [0.10, 0.10, 0.10, 1.0];
    for c in &b.m_curves_3d {
        let pts = sample_curve(c);                     // 64's adaptive sampler, reused as-is
...
Like 66's surfaces, BReps suppress `push_mesh`'s triangle-edge tubes — the edge *curves* are the
...
Nothing to add — 58's `Scene::apply_world_delta(row, &delta)` is type-blind: it rewrites the
...
```rust
    // commit (58): Scene::apply_world_delta(row, &delta) — set_xform + placed frame,
    //              no per-variant match, tess_cache untouched
```

And one line in reconcile (42b): the `changed` bucket must `tess_cache.remove(guid)` before
`apply_object` — an externally reshaped BRep re-tessellates once, exactly then. (This was already
65's invalidation rule; BRep just joins it.)
...
- **Click any face** of a multi-face solid → the *whole* BRep selects and the gumball sits at its
  ...
  hitch (cached; compare with 65's counter-experiment if you want to feel the difference).
...
```
Ch 66: iso lines — surfaces read as surfaces.
Ch 67: BREP, debts paid. (1) tess_cache absorbs BReps — render_mesh(guid) unifies the surface/BRep
       sources; the three direct b.mesh() sites (build, PICK-per-click, world box) now hit the
       cache.
       (2) Real edges: m_curves_3d (brep.rs:115) sampled with 64's adaptive sampler → tables.pipes
       ...
       (3) Transforms: 58's apply_world_delta is type-blind — nothing to add;
...
`68-trimmed.md` — the last citizen of Phase 10: `NurbsSurfaceTrimmed` — a surface with holes and cut
