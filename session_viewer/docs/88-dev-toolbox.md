# 88 Developer toolbox — selftest, surfaced errors, CI
...
fn main() {
    ...
    // 5. reconcile buckets (43b): 1 changed / rest unchanged on a perturbed copy
...
- `cargo run --example selftest --target x86_64-unknown-linux-gnu` → PASS lines, exit 0, no
  ...
  a bare `Rc::clone` (85's trap) → `copy identity` FAILs (the "copy"'s guid equals its source's)
...
Ch 88: THE WORKFLOW. Two visibility lines first: crate-type gains "rlib" (examples can't link a
       ...
       add_file invariants, BVH parity, a CPU pick, copy identity (85's duplicate-vs-Rc-clone
       trap), reconcile buckets — possible because picking was CPU-by-necessity (47) and Scene was
...
`89-web-polish.md` — the last mile: a progress bar for the 17.5 MB stress file, and a wasm binary
worth shipping.
