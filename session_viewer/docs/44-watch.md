# 44 Watch — external edits flow back in

> **Big picture.** *Phase 6 closes.* Load (34), save (43) — the last direction is external edits
...
> other tools instead of an import. The reaction is already built (42b's reconcile); what's new here
...
Second, **your own saves change the file too** (43), so a naive watcher would see its own write, reload,
...
<svg viewBox="0 0 680 170" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="every N frames the viewer re-fetches the file, hashes the bytes, and if the hash changed AND is not its own last save it runs 38 reconcile; otherwise it skips" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <rect x="404" y="76" width="150" height="24" fill="none" stroke="#6fb3ff"/><text x="479" y="92" fill="#d7dae0" text-anchor="middle">changed → reconcile (42)</text>
...
src/state.rs             # poll every N frames; self-write guard; apply_session factored out of 42b's reload
...
42b's `reload` fetches, parses, *and* applies. Watch needs the apply half on its own (its poll task
delivers an already-parsed `Session`), so split it — `pub`, because the message arm in lib.rs calls it:

```rust
    /// Diff `new` against the loaded scene and push only the changes to the GPU — the body of 42b's
...
42b's `reload` becomes: fetch + parse in its task, `apply_session(new)` when its message lands —
watch rides the exact same channel below.

> **Which doc is the watcher's?** 42b's reconcile is per-doc, but `Doc` stores `name`/`place`/`session`
...
> (its `file` is the url, its `name` names the doc), or — once 42b's reload lands a source url on `Doc`
...
Object-level `content_hash` (42) answers "which objects changed"; the watcher first needs a cheaper
...
The watcher is another frame-counter job (like 43's debounce): every `WATCH_POLL_FRAMES`, re-fetch and
compare. `watch_seen` remembers what a poll last *saw*; `self_write_hash` remembers what we last *wrote*
(set by 43's save). A change is real only if it differs from both.
...
```rust
    ...
    // add inside render(), after 43's `self.frame += 1;` — the poll: spawn a task that fetches,
...
**Close the loop with 43's save.** When Step 4 of lesson 43 writes bytes, stamp their hash so the very
next poll recognizes them as ours:

```rust
    // in 43's save trigger, right after building `bytes` and before/after download_bytes:
...
```rust
    Msg::Watched(url, new) => {
        ...
        state.apply_session(new);   // 42b's incremental diff — only what moved
...
- **External edit flows in.** With the viewer open on `floor_model.pb`, edit that file from another
  ...
  that one object updates. Nothing else redraws (42b's diff).
...
```
Ch 43: save — write the file out, gated by dirty + debounce + hash.
Ch 44: WATCH — external edits flow back IN, closing the 3-way sync. Browser can't watch a
       ...
       (fetched AND parsed in the poll task) → Msg::Watched on the loader's channel → 42b's
       reconcile → apply_session (the apply half of 42b's reload, factored so manual reload and
       the watcher share it) → only what moved hits the GPU. The SELF-WRITE GUARD
       (self_write_hash, stamped by 43's save) drops any change whose bytes equal our own last
...
from 42b's reload, the spawned poll task — fetch, guard, chunked parse — plus `self_write_hash` stamped
by 43's save), `lib.rs` (`Msg::Watched` + its `user_event` arm; proxy clone handed to `State`).

## Next

`45-screen-to-ray.md` — Phase 7 opens: picking. The mouse is a 2D point; everything selectable is 3D. The
