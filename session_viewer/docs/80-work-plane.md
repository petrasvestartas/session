# 80 Work plane — "the ground" becomes a choice
...
<svg viewBox="0 0 680 130" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="the work plane is stored state; the click resolver, grid, snap, and draw tools all read it; set by three points or a picked face" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g fill="#d7dae0" font-size="10">
    <text x="466" y="30">empty-click resolver (53) → ray ∩ plane</text>
    <text x="466" y="61">grid shader (70) → plane frame</text>
    <text x="466" y="92">rect/box (63), grid snap (64) → plane uv</text>
  </g>
...
`cursor_world_point` (53/64) swaps its hardcoded `z = 0` intersection for the plane:
...
A three-way verb, all Get-loop (54's grammar). The dispatch arm (`commands.rs` needs
...
impl ActiveCommand for CplaneFace {
    fn feed_point(&mut self, state: &mut crate::state::State, _p: Point) -> CmdStep {
        // the fed point is the snapped click; the FACE comes from 48's sub-object resolve
...
**`rect`/`box` (63) build in plane uv.** Corner clicks convert to plane coordinates, the rectangle
...
In `rect.rs` (63), find `rect_corners` → replace it and its callers' first lines:
...
**Snap (64).** The `Grid` candidate quantizes in plane uv instead of world xy; `Endpoint`/`Vertex`
snaps are world-space and unaffected. `snap()` gains a `plane: &Plane` parameter (the call site in
`cursor_world_point` passes `&self.work_plane`; `snap.rs` imports `Plane`). Find 64's grid-candidate
block (`if raw[2].abs() < 1e-6 { … }`) → replace with:

```rust
    // grid candidate — the crossing nearest the raw point, in PLANE uv (80)
...
`81-advanced-perf.md` — headroom for scenes beyond the stress file: LOD/decimation, occlusion
