...
row state — selection bits (50), hidden bits (51), cull bits (41), gumball live models (54-56),
reserved gumball/ghost rows (57, 63) — is ERASED by the next arriving file unless it also lands
...
`Session::ray_cast`/`world_xform` know NOTHING about `place`. Picking (47, 49), snapping (64),
work-plane (80), and any world↔local inverse must conjugate by the doc's `place`, or every hit
on a placed sheet is off by its manifest offset. Deltas committed as local `set_xform` must be
`place⁻¹ · delta · place` — cosmetic for translations, visibly wrong for rotations (60).
...
exactly those lessons. 74's selection mask must mirror all four lane sub-draws, not one.

**B6 — no runtime-add path yet.** `add_file` is the only table writer. Draw tools (62, 63, 65)
...
| 55 | gumball-rotate-scale | TOUCH-UP | inherits 59's commit prose; rotation about wrong origin if place not conjugated (B4) |
| 56 | gumball-numeric | **REWRITE** (Step 3) | `apply_transform_command` on `scene.session` + 59's dead `apply_delta`; Steps 1-2 fine |
...
- 66's whole thesis is now true by construction (shape cache can't see transforms).
...
- Multi-doc gives 75's tree panel its natural top level (one row per `Doc`).

## Stale reference docs

- `_LESSON_AUDIT.md` (2026-07-20): header scope wrong (74 is a full lesson now); ~12 listed bugs
  already fixed in the current lesson texts; 66's suggested fix uses the deleted `.xform`; the
