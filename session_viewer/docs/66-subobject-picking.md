# 66 Sub-object picking — vertex, edge, or face
...
> the gumball (65+) and control-point editing (85) possible later; the screen-pixel radius trick it
...
The exact inverse of 46's unproject: world → camera-relative (`− origin`) → clip (`view_proj`) → NDC →
...
the first face that contains it wins. It's the one piece 47's ray-cast couldn't hand us — the kernel's
...
`67-pick-thin-geometry.md` — Lines, polylines, and points are 1D and 0D: a ray passes *through* them,
