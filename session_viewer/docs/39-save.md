...
`40-watch.md` — the third sync direction: an *external* edit to the file flows back in. The browser can't
watch a filesystem, so a File System Access handle polls `lastModified` (or a watcher→WebSocket bridge
pushes), and on change we run 38's `reconcile`. The catch is the loop: our own Step-4 save changes the
