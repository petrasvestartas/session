# 83 Developer toolbox — selftest, surfaced errors, CI
...
natively in milliseconds. The archive proved this pattern (`cargo run --example selftest`); ours
asserts the invariants the lessons established:

```rust
//! Headless viewer selftest: no browser, no GPU. `cargo run --example selftest`
//! Exit 0 = all pass. Run it before every push; CI runs it on every push.

use session_viewer::app::scene::Scene;
use session_rust::{Session, Point, Vector};
...
fn main() {
    ...
    // 2. Scene invariants: rows bijective, boxes finite, BVH == brute force (36's parity)
    ...
    // 4. reconcile buckets (38b): 1 changed / rest unchanged on a perturbed copy
...
other reasons; testability fell out. (Small enabler: the crate needs `[lib]` alongside the wasm
binary so examples can `use session_viewer::…` — a 3-line `Cargo.toml`/`lib.rs` visibility touch.)
...
3. **Counts** — HUD objects vs expected (34a logs what loaded). `0 objects` = load/parse problem,
   not rendering.
...
jobs:
  build:
    ...
    steps:
      ...
      - run: cargo run --example selftest        # headless invariants, seconds
...
`std::fs` in browser paths, missing web-sys features). No headless-browser GPU testing — WebGPU in
CI containers is still fragile; the CPU selftest plus a compiling wasm covers what's coverable.

## Step 5 — verify

- `cargo run --example selftest` → PASS lines, exit 0, no browser anywhere. Break something real
  (comment `refresh_guid` out of `clone_selection`) → a FAIL and exit 1.
...
```
Ch 82: the viewer answers questions about the model.
Ch 83: THE WORKFLOW. Headless selftest (examples/selftest.rs): fixture counts, Scene invariants,
       BVH parity, a CPU pick, reconcile buckets — possible because picking was CPU-by-necessity
       (42) and Scene was GPU-free by design (35); seconds, exit-coded, no browser. GPU errors:
       on_uncaptured_error + per-pipeline error scopes → a static queue → the CLI log, with the
       pipeline NAMED. The black-screen checklist: log → draw count → object count → camera(F!) →
       clear-color → bisect passes. CI: selftest + trunk build --release; no headless-GPU testing
       (fragile), and honestly so.
```

Edited: `examples/selftest.rs` (NEW), `engine/gpu/mod.rs` (error scopes + queue), `Cargo.toml`
(lib target for examples), `.github/workflows/viewer.yml` (NEW).

## Next

`84-web-polish.md` — the last mile: a progress bar for the 17.5 MB stress file, and a wasm binary
worth shipping.
