# 70 Pick thin geometry — lines and points need a radius
...
> 47's triangle test can't ever hit one. Every CAD app solves this the same way: give the pick a
...
## The plan — reuse the kernel's cast, keep 47's mesh path
        ...
        for doc in self.docs.iter_mut() {
            ...
            for h in hits {
                match doc.session.lookup.get(h.guid()) {
                    // The kernel's BRep arm is a no-op and 47's pick_mesh owns solids anyway.
...
every reconcile (49) or regen, so the FIRST click after one pays a full rebuild per doc — a visible
...
## Step 3b — curves join the thin pick

`Session::ray_cast`'s curve arm is a deliberate no-op (exact ray↔NURBS is out of scope
kernel-side), so curves pick as what they DRAW as: ray↔segment over 43's CACHED samples —
pick what you see, with this lesson's screen-radius tolerance. In `pick_thin`, find its
final `None` (after the kernel-cast loop) → replace with (it runs only when the kernel
cast matched nothing):

```rust
        // curves: ray↔segment over the cached samples (57's tolerance, same tol)
        let mut best: Option<PickHit> = None;
        for doc in &self.docs {                          // multi-doc (55): EVERY doc's lookup —
            for (guid, geom) in &doc.session.lookup {    // the singular `session` walk was stale
                ...
                for w in pts.windows(2) {
                    ...
                    if let Some(hit) = session_rust::intersection::line_line(
                        // 1.0e7 = 54's far-point cap — beyond it this pick ray ends early
...
course's scene sizes; when it isn't, the broad-phase is 52's BVH (gather candidate guids with
`objects_in` around the ray's box, then test only those cache entries — the same pattern 72's
...
ray) first, exactly like 55's mesh arm does; at the identity placements this phase uses, the code
...
`71-selection.md` — picking finds objects; selection *keeps* them. `FLAG_SELECTED` in the instance row
