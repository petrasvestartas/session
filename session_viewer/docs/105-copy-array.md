# 105 Copy, duplicate, array — daily-use editing, nearly free
...
> geometry type), **doc-aware insertion** (70's `add_object` rail, wrapped in one `AddGeometry` so
...
<svg viewBox="0 0 680 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="copy duplicates the selection minting fresh guids, inserts the batch into each source doc as one AddGeometry command, then places every copy with apply world delta" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g fill="#d7dae0" text-anchor="middle">
    ...
    <text x="417" y="47">ONE AddGeometry (70)</text><text x="417" y="59" fill="#666" font-size="9">insert batch → one undo</text>
  ...
  <text x="340" y="100" fill="#888" text-anchor="middle">array = the same pipeline in a loop; Alt+gumball = the same pipeline on 67's release</text>
...
`Geometry` variants hold **`Rc<T>`** (64). So `lookup.get(guid).cloned()` clones the *handle*: the
...
- **its doc** — `guid_to_row` + `doc_of_row` (52). Copies go into the **source object's doc**, so
...
`impl AddGeometry` block (70) and add this beside `one` (`RemovedObj` is 64's snapshot shape —
...
```rust
    ...
    /// `one`, plural. Each snapshot inserts through 70's add_object path into ITS OWN doc.
...
```rust
    ...
    pub fn clone_selection(&self) -> Vec<RemovedObj> {
        let mut out = Vec::new();
        for &row in &self.selected {                        // row-keyed (58) — guid at the edge
...
their rows resolve through `guid_to_row`, and each gets 67's commit primitive plus the live GPU
poke:

```rust
    /// Post-insert placement for a batch of copies: 67's apply_world_delta per new row
...
editing one copy's CVs (85) can't move the others.

But the common case never edits them, and the architecture already owns the cheaper shape: 29–31's
instancing draws N rows from ONE vertex payload, and 46's content fingerprints already detect
...
a real dependency: an edit must COW the shared slot back into a private one (the same split 85's
...
`CopyCmd` is 70's `LineTool` shape exactly — a struct with `from: Option<Point>`, the first click
stashes it, the second runs the batch: duplicate → ONE `AddGeometry` (the insert routes each
snapshot through 70's `add_object` into its own doc and `set_xform`s the carried local — the copy
...
(Snap (72) applies to both picks automatically, so copy-from-corner-to-corner is *exact*.)
...
One branch on 67's **release**, not its press: the drag runs exactly as 59 built it (live,
...
```rust
        // 67's release handler, first lines:
        if let Some(ctx) = self.gb_drag.take() {
            ...
            // …the normal commit continues unchanged (67's apply_world_delta + TransformObjects)…
        }
```

The subtlety that makes this clean: 67's live drag **never mutates the kernel objects** — so
...
- **Alt+drag** the X arrow → the original stays put, release → a copy at the drop point, one undo
  step. Without Alt → 67's normal move, unchanged.
...
```
Ch 84: files in and out.
Ch 85: DUPLICATION = three rails composed: duplicate() + AddGeometry + apply_world_delta. The trap:
       ...
       AddGeometry::of_snapshots (inserts via 70's add_object into the SOURCE doc; undo removes the
       batch) → place_copies: apply_world_delta per new row (rows exist post-insert, via
       guid_to_row) + the live poke. array = the loop form, still one Command. Alt+gumball = 67's
...
`106-layers.md` — organization users expect: layers — which the PDF importer already built as tree
