# 85 Layers — naming what the importer already built
...
> group per OCG layer, per doc** (pdf.rs — `add_group` per named CAD layer), 74's panel renders
> them with eye icons, and branch visibility resolves to 50's set operations. What's missing is
...
74's identity rule decides everything here. In the kernel tree, `node.name` is the document
...
```rust
    ...
    /// fed to 50's hidden set. Object guids live in node.NAME (74's identity rule).
...
New objects land on the active layer with a one-line change where 61's `add_object` parents its
tree node: pass `state.scene.active_layer` — resolved at that moment via
`layer_node(d, name)` with `d` = the target doc (61's `active_doc`) — as the `parent` argument
...
```rust
        "layer" => match (parts.next(), parts.next()) {
            ...
            (Some(dir @ ("off" | "on")), Some(name)) => {
                // fold over the docs, union the descendants, hand them to 50's set ops
...
- `layer list` → the *imported* layers, by name, with member counts — Bemassung, Schraffur, … —
  exactly the groups 74's panel shows per sheet, now summed across docs. Nothing was created;
  the import did this.
- `layer off Bemassung` → dimensions vanish from **every sheet at once** — draw AND pick (the
  cross-doc fold feeding 50's one authority). `on` → back. 74's per-sheet branch eyes still work
  independently — same `hidden` set, two entrances. Undo history untouched — visibility isn't a
  document mutation (50's rule).
...
- Save (43) → reload → the layer structure round-trips **per doc**: each doc's `.pb` carries its
...
```
Ch 84: duplication.
Ch 85: LAYERS = tree groups the PDF importer ALREADY builds (one per OCG layer, per doc) — the
       lesson names them. A layer is a NAME, not a node: the same name is a different group in
       every doc, so every op folds over scene.docs + get_node_by_name (74's identity rule — group
       ...
       active_layer: Option<String> = a name, resolved per target doc (61's active_doc) into the
...
`86-measure.md` — `probe` grows up: distance, angle, radius, object info — and a status bar that
