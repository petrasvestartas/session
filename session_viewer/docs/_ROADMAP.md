...
- 34h Colors & widths — honor what the user set (one resolution rule, CPU-side)
  ...
  - consumers: 49 selection (tint), 47 thickness slider (scales all multiplier widths), PointCloud
...
- ✅ 42 Reconcile (split: 38a per-object GpuArena, 38b diff by guid) — never rebuild the whole scene
...
- ✅ 43 Save (viewer → file) — write only when something actually changed
...
- ✅ 44 Watch (file → viewer) — external edits flow in
...
- ✅ 45 Screen → ray — unproject the mouse into a world ray
...
- ✅ 46 Ray-cast meshes — nearest hit wins
...
- ✅ 47 Sub-object picking — vertex / edge / face
...
- ✅ 48 Pick thin geometry — lines & points are 1D/0D, rays never hit them exactly
...
- ✅ 49 Selection highlight & marquee — see what you selected
...
- ✅ 50 Hidden-object filter — visibility is real state
  - steps: per-object visible flag (35) respected by draw (row collapsed/culled), picking (skip),
    and marquee; `hide`/`show` groundwork for the CLI verbs (52)
...
start instead of being retrofitted at lesson 73 like the old plan.
- ✅ 51 egui overlay + perf HUD + first settings
...
- ✅ 52 Command bus + Get-loop — THE interface arrives
  ...
  - steps: `enum GetState { Idle, WaitingPoint(prompt), WaitingOption(..) }`; a running command
    ...
    one prompt; every mutation goes bus → kernel → `Session` → reconcile (42); first verbs:
    `hide`/`show`/`zoom`
  - verify: type `hide` → selection hides; unknown verb → friendly message in the CLI log
- ✅ 53 Command options & modal multi-step
...
- ✅ 54 History & autocomplete — ↑/↓ recall, Tab prefix-complete, alias table (`l` → `line`)
  - verify: ↑ recalls last command; `li<Tab>` completes; aliases dispatch
- ✅ 55 Delete + undo/redo — the FIRST scene mutation, so ARCHITECTURE pattern (a) lands HERE
...
- ✅ 56 Gumball geometry — the 3-axis gizmo appears (reference_gumball_widget has the full recipe)
...
- ✅ 57 Gumball scale & hit-test — constant screen size + pickable handles
...
- ✅ 58 Drag to translate — first real transform
  ...
  - steps: on press snapshot; drag = closest-point-on-axis delta (f64 ray math); LIVE = matrix-only
    ...
    before/after snapshots → undoable for free (55)
...
- ✅ 59 Rotate + scale handles — arcs → angle about axis; boxes → (uniform) scale; same commit path
  - verify: rotation snaps visually to the arc plane; undo/redo across mixed drags works
- ✅ 60 Numeric entry — click a handle, type `500`, exact move (reuses the Get-loop input, 48;
...
- ✅ 61 Draw tools I — ARCHITECTURE pattern (b) lands: creating geometry
  ...
  - steps: `trait Tool { on_click / on_move / preview / finish → Box<dyn Command> }` + a ToolHost
    ...
    the Get-loop prompts (52); finish yields `AddGeometry` → undoable for free
  - verify: `line` command: click-click → line exists in the Session, undo removes it
- ✅ 62 Draw tools II — `PolylineTool` (N clicks, Enter finishes), `RectangleTool`/`BoxTool` (on
  the z=0 plane until 78's work plane); ghost preview via a transient instance row on `on_move`
  - verify: preview follows the cursor, never enters the Session until finish
- ✅ 63 Snapping — drawing becomes precise (was lesson 78 — belongs WITH drawing)
...
- ✅ 64 NurbsCurve — evaluate + draw
  - steps: kernel `NurbsCurve` sampled by parameter (adaptive count from span count) → polyline →
    the 31 cylinder path; `NurbsCurveTool` (N control clicks + Enter) via the Tool trait (61)
  - verify: curve renders smooth at every zoom; tool-drawn curve undoes as one Command
- ✅ 65 NurbsSurface — tessellate to a mesh (reference_viewer_nurbs_brep_pipeline)
...
- ✅ 66 Iso-curve boundaries — surface edges + iso lines through the 23/31 line path
  - verify: boundaries hug the surface with no z-fight (uses 23's depth bias)
- ✅ 67 BRep — faces + edges as one object; transform matrix-only (project_viewer_edge_brep_fixes)
  - verify: pick selects the whole BRep; edges + faces move together under the gumball
- ✅ 68 Trimmed surface — first-class `NurbsSurfaceTrimmed` (reference_viewer_trimmed_firstclass:
...
- ✅ 69 Analytic ground + infinite grid — white ground with distance fade
...
- ✅ 70 Render-on-demand — the single biggest perf win, and it never touches the image
  ...
  - the frame is IDENTICAL whether drawn during a drag or after — this lesson changes WHEN we
    draw, never WHAT we draw; the outline mask re-raster (73) stops burning battery on static
    scenes for free
...
- ✅ 71 GTAO — half-res, CONSTANT quality (replaces the archive's 48-tap always-on SSAO)
  ...
  - port from the archive (proven, don't re-derive): view-pos reconstruction via ANALYTIC
    inv_proj (`Xform::inverse` WAS affine-only — found & fixed kernel-wide in lesson 45), IGN noise,
...
- ✅ 72 Arctic + cheap global illumination — a better DEFAULT look for the same money
  - steps: arctic ambient (0.72..1.0 hemisphere) × AO upgraded to: sky visibility from the BENT
    ...
    the 8-bit swapchain; B toggle + settings checkbox (51)
  - verify: default look visibly improves (contact micro-shadow); arctic reads ≥ archive; zero
    extra texture fetches vs 67
- ✅ 73 Selection outline + AA polish — the archive look without the per-frame tax
  - steps: coverage-mask outline (4× MSAA fractional coverage, sharp 1px ramp — archive
    technique; 24's MSAA pays off here) BUT mask passes render only when dirty (70), and the
...
- ✅ 74 Scene tree — the Session's tree in a panel (reference_viewer_tree_undo)
...
- ✅ 75 Tree ↔ viewport — select in tree ⇄ highlight in viewport; auto-reveal-on-pick (expand +
...
- ✅ 76 Text labels — billboarded glyph text (archive `text.rs`: glyph atlas + TextVertex quads)
  - verify: labels face the camera, readable at the four named views

## Phase 13 — Sub-object editing & polish
- ✅ 77 Control-point edit — F10 mode (reference_viewer_subobject_edit)
  - steps: sub-pick (47) grabs verts/CVs; gumball moves them; **partial GPU update**
...
- ✅ 78 Edit points (Greville) — reshape curve/surface via R⁻¹ refit, weights kept
...
- ✅ 79 CAD plane / work plane — construction plane (cad_plane.rs): set by 3 points / to-object
  (CLI command, 48); draw tools (61/62) and grid snap (63) target the active plane
  - verify: rectangle drawn on a tilted work plane lands in that plane
- ✅ 80 Advanced perf — LOD/decimation, occlusion culling, GPU compute cull + indirect draw
  (culling + batching already landed in 30/37; 27 unlocked compute)
  - verify: perf HUD before/after on the capstone scene
- ✅ 89 (optional appendix) Materials & textures — `docs/89-textures.md`. Written against the real
...
- ✅ 82 Section / clipping planes — THE missing CAD feature (AEC scenes demand cuts)
  - steps: N world clip planes in a uniform; every geometry fs gains `if (dot(p, plane) < 0) {
    ...
    or from the work plane (79), gumball-draggable along its normal; optional: darker "cut" tint on
    back faces as a cheap cap illusion (true caps = kernel plane-splits, later)
  - picking must respect the cut: 46's local-frame cast filters hits behind active planes
  - verify: section a floor model wall — the inside reads; drag the plane through the building live
- ✅ 83 Import / export OBJ + STEP — the kernel codecs are ALREADY THERE (file_obj, file_step,
  ...
  - steps: extend 34a's extension dispatch (`.obj`/`.step` → the kernel decoders) + `<input
    type=file>` open (34a's noted alternative) + `export obj|step` verbs via 43's download path
  - verify: drag a real .step in → tessellated BReps appear; export → reopens in FreeCAD/Rhino
- ✅ 84 Copy / duplicate / array — daily-use editing (trivially cheap on this architecture)
  - steps: `copy` verb = clone selection, fresh guids, offset via Get-loop point pair →
    AddGeometry (61) so undo is free; Alt+gumball-drag = drag a copy (58's skeleton, clone at
...
- ✅ 85 Layers via tree groups — CAD organization users expect (Session::add_group already exists)
  - steps: group nodes get eye/color chips in the tree (74); branch visibility = 50's set ops;
...
- ✅ 86 Measure + status bar — `probe` (53) grows up: `distance`, `angle` (3 points), `radius`
...
- ✅ 87 Developer toolbox — the workflow lesson (headless selftest + debugging)
...
- ✅ 88 Web polish — load-time & size (the 17.5 MB stress file over a real network)
...
## Capstone
- ✅ 81 Load the floor model — the compas_tf demo as a first-class scene
  - steps: load the `.pb` (34/42), fit (15), run the full loop: pick → tree reveal → gumball →
...
7. **Virtualize the UI.** Build only visible tree rows / labels (70).
...
Measure, don't guess: a console fps/draw-call counter lands early (29) → egui HUD (68).
...
- **Single source of truth = the in-memory document** (`Session`), not the file; the GPU scene is
  a *view* of it. **Update = delta (42), not reload** — diff by `guid`, like virtual-DOM / asset
  hot-reload / Vite HMR / CRDT-Git. **Write only when changed (43).** **No feedback loop (44)**
