# 49 Command options — modal, multi-step, Rhino-style
...
```
Ch 48: the bus — verbs dispatch, instant commands work end to end.
Ch 49: THE CONVERSATION. CmdOption { Toggle / Number / List } renders into the Rhino prompt shape —
       ...
       `x,y,z` converges on feed_point, empty Enter = default, Esc = cancel (48). Geometry-free on
...
`50-history-autocomplete.md` — the CLI grows muscle memory: ↑/↓ recall previous commands, Tab
prefix-completes verbs, and an alias table (`l` → `line`) formalizes what 48's match patterns started.
