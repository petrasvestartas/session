# 85 Draw tools I — creating geometry
...
> a multi-step command** (61's `ActiveCommand` — prompts, clicks, Esc) whose final act is an
...
Adding and deleting are the same operation with time reversed — 64's `RemoveObjects` already knows
...
Two tiny additions to `RemoveObjects` in `src/app/history/remove.rs` (64) — add both inside its
`impl RemoveObjects`, beside `of_selection`:

```rust
    /// Wrap ready-made snapshots (70's AddGeometry; 92's copy batches reuse it).
...
destructure from 64's note, factored once):
...
```rust
    pub fn commit(&mut self, cmd: Box<dyn crate::app::history::Command>) {
        ...
        scene.rebuild_bvh();   // new/removed objects change the box set (52)
    }
```
...
> by design (52/54), but two rules follow: only the *changed* rows' `world_boxes` need
> recomputing before the rebuild (the 46 pattern), and batch creators must not commit per
...
> sit in the history stack until evicted. 64's undo-memory section (stack cap, coalescing,
...
impl ActiveCommand for PointTool {
    ...
    fn feed_text(&mut self, state: &mut crate::state::State, s: &str) -> CmdStep {
        // 'x,y,z' typed — same convergence as probe (62)
...
Structurally `ProbeCmd` (62) with a different `Done`:
...
Remember 61's empty-click rule — a click on bare grid intersects `z = 0` and still yields a point —
...
- **`pt`** ⏎, type `100,200,0` ⏎ → a point object lands at exactly those coordinates — typed and
  clicked input converge, as designed in 61. Type `100,200` → the log tells you what was wrong
  (`expected 'x,y,z' numbers`) and re-asks; bad input is never eaten silently.
- **Save** (50) after drawing → reload the downloaded file → your drawn line is in it. It was a
...
```
Ch 69: numeric entry — the gumball is complete.
Ch 70: CREATION = pattern (b), and it's small on purpose: a drawing tool IS an ActiveCommand (61/62's
       machinery: prompts, click-or-type convergence, back, Esc) whose finish COMMITS an AddGeometry
       (64's machinery: undo). AddGeometry = RemoveObjects mirrored — apply calls revert and vice
       versa; one tested insert/delete body, two directions. No DrawTool enum (same dead-end as
       UndoAction — every new tool would grow a central match); State.active from 61 IS the
       ...
       Drawn objects are Session-first-class: they save (50), diff (49), pick (55), and undo
...
`86-draw-tools-2.md` — tools that need *N* clicks and live feedback: `PolylineTool` (Enter finishes),
