# 78 History & autocomplete — the CLI grows muscle memory
...
```rust
    pub cli_history: Vec<String>,        // ← ADD to State (init empty in State::new)
                                         //   NB: named cli_history, not history — lesson 79 adds
...
`79-delete-undo.md` — the first *destructive* mutation, and with it the pattern the whole editor
