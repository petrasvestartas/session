# 80 Advanced perf — headroom beyond the stress file

> **Big picture.** *Phase 13 closes.* The viewer already carries its two acceptance scenes easily —
> batching (30/31), culling (41), render-on-demand (70), and the half-res AO (71) did the heavy
...
<svg viewBox="0 0 680 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="three levers: lod swaps cheaper meshes by screen size, occlusion culling skips hidden objects, gpu cull writes indirect draw arguments with zero cpu involvement" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g fill="#666" text-anchor="middle" font-size="10">
    ...
    <text x="340" y="62">behind a wall → skip entirely</text><text x="340" y="76">depth-pyramid test vs 41's frustum</text>
...
- **Levels:** per mesh, precompute 1–2 decimated versions (the kernel side: cluster-collapse or
  ...
  `arena.allocate` each level at load. Cheap: slots are just ranges (42a).
- **Selection:** projected box height in px (the world box through 47's `project_to_screen`) picks
...
- Test each object's world box against the pyramid at the mip where the box is ~2 px: if the box's
  nearest depth is farther than the stored farthest depth, nothing of it can show — set 41's
...
compute pass (one thread per object):
    read  objects[]   (boxes, in a storage buffer — 40's data, GPU-resident)
    test  frustum planes (41's math, in WGSL) + depth pyramid (lever 2)
...
Every lever above starts with the same sentence: *profile first*. The HUD (28/41/70) already shows
...
- Confirm the baseline numbers on the acceptance scene — the 10-sheet manifest
  (`scenes/drawings.json`) — and write them into `_ROADMAP.md`'s capstone entry — the capstone (81)
...
```
Ch 79: work plane — Phase 13's modeling tools complete.
Ch 80: HEADROOM, mapped. (1) LOD: decimated levels pre-allocated as extra arena ranges; screen-size
...
`81-capstone.md` — everything, together: load the floor model, run the whole loop — pick, tree,
