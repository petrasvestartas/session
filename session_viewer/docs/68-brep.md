...
> on every use** (34's noted caveat, 47's pick does it *per ray*), its **edges** draw as tessellation
...
priming pass at the top of `add_file`, 67's warm/read sites) to `render_mesh` — same signature, 62
...
fn brep_linework(b: &BRep) -> Vec<CylinderSegment> {
    ...
    for c in &b.m_curves_3d {
        let pts = sample_curve(c);                     // 65's adaptive sampler, reused as-is
...
Like 67's surfaces, BReps suppress `push_mesh`'s triangle-edge tubes — the edge *curves* are the
...
Ch 68: BREP, debts paid. (1) tess_cache absorbs BReps — render_mesh(guid) unifies the surface/BRep
       ...
       cache.
       (2) Real edges: m_curves_3d (brep.rs:115) sampled with 65's adaptive sampler → tables.pipes
