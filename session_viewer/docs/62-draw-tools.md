# 62 Draw tools I — creating geometry
...
Adding and deleting are the same operation with time reversed — 56's `RemoveObjects` already knows
...
Two tiny additions to `RemoveObjects` in `src/app/history/remove.rs` (56) — add both inside its
`impl RemoveObjects`, beside `of_selection`:

```rust
    /// Wrap ready-made snapshots (62's AddGeometry; 85's copy batches reuse it).
...
destructure from 56's note, factored once):
...
Structurally `ProbeCmd` (54) with a different `Done`:
...
Remember 53's empty-click rule — a click on bare grid intersects `z = 0` and still yields a point —
...
`63-draw-tools-2.md` — tools that need *N* clicks and live feedback: `PolylineTool` (Enter finishes),
