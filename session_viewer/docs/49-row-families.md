...
> Lesson [57](57-trimmed.md) adds a trimmed surface whose edges are already `CylinderSegment`s,
> [64](64-screen-to-ray.md) picks against those same rows, [112](112-meshlets.md) re-batches them.
...
```bash
python3 docs/_replay_check.py --moves <end-of-47 tree> /tmp/w48 docs/49-row-families.md
```

```text
docs/49-row-families.md: 76 ops, 0 failed
docs/49-row-families.md: 1 move source(s), 0 not byte-identical
```
