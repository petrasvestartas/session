# 86 Measure + status bar — the viewer answers questions

> **Big picture.** *Phase 14.* CAD models exist to be interrogated: how long, how far apart, what
> angle, how big. Lesson 57's `probe` was secretly the prototype — a Get-loop conversation ending in
...
Each is 53's template with a different `Done`. Build each as a `ProbeCmd`-shaped struct (53) that
...
Every point in these conversations comes through `cursor_world_point` — **snapped** (63). Measuring
...
nothing needs snap, resolve the readout against the work plane only (79's one intersection) —
...
```
Ch 85: layers.
Ch 86: INTERROGATION. distance/angle/radius = 53's conversation template + kernel math (distance,
       Vector::angle, Heron circumradius with a collinear guard); every picked point arrives SNAPPED
       (63), which is what makes measurements exact rather than approximate. `what` = instant verb
...
`87-dev-toolbox.md` — the developer-experience lesson: a headless selftest, GPU errors that surface
