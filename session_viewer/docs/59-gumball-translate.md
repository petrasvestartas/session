# 59 Gumball III — drag to translate
...
<svg viewBox="0 0 680 130" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="press stores state, movement past a threshold begins the drag, each mouse move applies a matrix-only delta live, release commits a TransformObjects command onto the undo stack" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g fill="#d7dae0" text-anchor="middle">
    ...
    <text x="215" y="47">travel ≥ 4 px?</text><text x="215" y="59" fill="#666" font-size="9">else: click (61)</text>
...
One method, `pub` — 60's rotate/scale, 61's typed entry, and 85's copy-array all call it. No
...
fingerprints geometry — 43b's reconcile, 44's save-if-changed — must fingerprint
...
**Two small helpers first** — both in `impl State`. `cursor_ray` is 46's unproject factored into a
method (the drag needs a ray on every mouse move); `begin_drag` fills the `DragCtx`:

```rust
    /// 46's screen_to_world_ray with its three locals, factored (47's click site builds the same).
    ...
    fn begin_drag(&mut self, handle: HandleKind) {
        ...
        let axis = match handle {
            ...
            _                             => Vector::z_axis(),   // Z handles + ScaleUniform (60)
        };
...
**Mouse move** — begin past the threshold, then live-update (in the cursor-move handler, after 58's
hover block):
...
`60-gumball-rotate-scale.md` — the same skeleton, two new deltas: arcs → ray-plane intersection +
