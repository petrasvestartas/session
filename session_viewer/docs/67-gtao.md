# 67 GTAO — ambient occlusion at constant quality
...
(fullscreen) multiplies scene × AO into the swapchain — the standard post-process arrangement; 69's
...
position from depth. The tempting `inverse(proj)` upload hits 41's trap in shader form — and even a
...
- Perf: on an iGPU, orbiting the stress file with AO on holds interactive rates, and idle is still
  **0 drawn/s** (66) — the entire AO cost rides only on frames that draw anyway.
...
`68-arctic-gi.md` — the look, completed: sky-visibility from the bent normal (creases darken toward
