# 69 Analytic ground + infinite grid — the stage
...
fn fs_main(in: VsOut) -> FsOut {
    // Unproject this pixel at two depths → the camera ray (45's math, on the GPU, camera-relative).
    let n4 = g.inv_view_proj * vec4<f32>(in.ndc, 1.0, 1.0);    // reverse-Z near
    let f4 = g.inv_view_proj * vec4<f32>(in.ndc, 0.5, 1.0);    // well-conditioned far (45's rule!)
    let p0 = n4.xyz / n4.w;
...
<svg viewBox="0 0 520 148" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="GroundUniform byte map: a vec3 pad forces 16-byte alignment leaving an 84 to 96 hole and inflating the WGSL struct to 112 bytes versus the 96-byte Rust mirror; three scalar pads fix it" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <text x="222" y="50" fill="#888" text-anchor="middle">inv_view_proj (68)</text>
  ...
  <text x="222" y="114" fill="#888" text-anchor="middle">inv_view_proj (68)</text>
...
45's conditioning rule applies on the GPU too — and everything is **camera-relative** (the uniform
...
4×4 inverse — fixed during lesson 45; this matrix contains the projection, which the old affine-only
...
- Perf HUD: +1 draw call, fullscreen but almost branchless — frame time moves barely at all. The
  stage is set for the money lessons: render-on-demand (70) and GTAO (71).

## Recap

```
Ch 68: Phase 10 closed — all geometry first-class.
Ch 69: THE STAGE. Analytic ground: a fullscreen triangle whose FRAGMENTS each unproject their pixel
       (ndc_z 0.5 far — 45's conditioning rule holds on the GPU), intersect z=0 in CAMERA-RELATIVE
...
`70-render-on-demand.md` — the single biggest perf win in the whole course, and it never touches the
