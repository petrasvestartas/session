...
impl Gpu {
    ...
    pub async fn new(window: std::sync::Arc<winit::window::Window>) -> anyhow::Result<Self> {
        ...
        let bunny = session_rust::read_xyz_from_str(include_str!("../../../session_data/bunny.xyz"));
        ...
        for p in bunny.get_points(){
            points.push(CloudPoint { 
                position: p.to_f32(), 
                instance_id: cloud_ri, // every point share the cloud's one mode matrix
