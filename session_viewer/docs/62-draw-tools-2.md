# 62 Draw tools II — polyline, rectangle, box, and the ghost preview
...
> A tool shows you a possible future; only `finish` makes it real. Get that boundary wrong and 43's
...
Identical mechanism to the gumball's overlay tables (56), one level simpler — no depth-clear pass
needed, ghosts draw in the main cylinder pass, just from their own small buffer. Three new `struct
Gpu` fields, below 56's `gb_*` block:

```rust
    // ghost preview table (62) — replaced whole on every tool mouse-move
...
In `Gpu::new`, find 56's reserved-row block (`let gb_row = objects_base.len() as u32;` …) → insert
after it:
...
and find 56's `gb_glyph_bind_group` creation → insert after it (same fixed-capacity trick):
...
locals.) The upload pair mirrors 56's, with one extra move: `set_preview` **stamps every segment
...
```rust
            // ghost preview (62) — same pipeline, its own table at group 3. Fully self-bound:
...
In `state.rs`, the point resolver becomes a named method — 52's click reroute computed
"pick-or-z=0" inline; motion needs the same answer every mouse-move, so factor it now. Add to
`impl State` (58's `cursor_ray` sits right above it):

```rust
    /// THE point resolver: the world point under the cursor — scene hit if any, else ray ∩ z=0
    /// (41 Step 3's formula). 52's clicks and this lesson's on_move both call it; 59 adds snap,
    ...
    fn cursor_world_point(&mut self) -> Option<Point> {
        ...
        // 48's pick-site tolerance trio, verbatim
...
(52's `WaitingPoint` click branch now reads `if let Some(p) = self.cursor_world_point()` and feeds
...
`feed_text(state, "")` (52) — polyline uses that as "finish".
...
(Esc must also clear the ghost — add `state.gpu.clear_preview()` to 52's cancel arm, once, centrally:
...
impl ActiveCommand for BoxTool {
    ...
    fn feed_text(&mut self, state: &mut crate::state::State, s: &str) -> CmdStep {
        if self.corners.len() == 2 {
            // 53's grammar: a typed value at a point prompt — here, the height
...
Register the verbs — in `dispatch()`'s match (`app/commands.rs`), beside 61's `"point"`/`"line"`
arms:
...
- The leak test: mid-polyline with 3 ghost points, hit **Save** (Ctrl+S if wired, or wait for 43's
  debounce) → the saved file contains *no trace* of the ghost. Esc → ghost gone, nothing committed.

## Recap

```
Ch 61: creation — tools are ActiveCommands, finish = AddGeometry.
Ch 62: N-CLICK TOOLS + THE GHOST. Preview = a dedicated small segment buffer (gumball's table
...
`63-snapping.md` — drawing becomes *precise*: endpoints, vertices, and grid intersections within a
