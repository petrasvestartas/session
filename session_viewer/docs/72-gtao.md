# 72 GTAO — ambient occlusion at constant quality
...
position from depth. The tempting `inverse(proj)` upload hits 46's trap in shader form — and even a
...
- Perf: on an iGPU, orbiting the stress file with AO on holds interactive rates, and idle is still
  **0 drawn/s** (71) — the entire AO cost rides only on frames that draw anyway.
...
`73-arctic-gi.md` — the look, completed: sky-visibility from the bent normal (creases darken toward
