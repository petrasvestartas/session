...
fn fs_main(in: VsOut) -> FsOut {
    // Unproject this pixel at two depths → the camera ray (46's math, on the GPU, camera-relative).
...
4×4 inverse — fixed during lesson 46; this matrix contains the projection, which the old affine-only
...
`71-render-on-demand.md` — the single biggest perf win in the whole course, and it never touches the
