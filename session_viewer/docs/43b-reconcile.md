...
verbs, and the `Geometry` match — converting an object to those GPU types via 43a's `flatten_mesh` /
...
```rust
    ...
    /// rows. Same shape as 43a's ensure_seg_capacity; needs `instance_layout` hoisted onto Gpu.
...
`44-save.md` — the reverse trip. An in-viewer edit marks the object dirty; a debounce coalesces the
