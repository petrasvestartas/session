# 89 GPU surfaces — tensor product in compute, the arena gets a producer
...
> triangle pass already draws. 44's economics survive with one word changed: *tessellate once*
...
version was the reason 44 needed a cache at all.

The upload rule from 73 extends: CVs go up **v-major exactly like `m_cv`** and
...
`find_span` and the Cox–de Boor triangle are 73's functions parameterized by direction — port
...
const MAX_ORDER: u32 = 8u;   // 73's cap - the ported basis arrays size against it here too
```
...
/// Uniform-in-span like 73; clamped so one surface can never starve the arena.
...
the entry keeps feeding 55's raycast at a fixed coarse resolution; the display path never
touches it again. (Same CPU-proxy split as 73, stated once more because it is the load-bearing
reason Phase 10b can precede nothing in 40–50.)

## What you should see

A surface (a loaded file) shades exactly as before — the acceptance diff:
...
```
Ch 74: the producer pattern, second table. CV grid + two knot vectors upload once; one
...
`90-gpu-trimming.md` — the lesson the CDT does not survive: trim loops become a per-fragment
