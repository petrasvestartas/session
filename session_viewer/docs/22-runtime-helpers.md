# 22 · Refresh diagnostics and resource checks
...
`lessons/22/src/engine/gpu/device.rs` · edit · type this

Replaces the line `#[cfg(target_arch = "wasm32")]` in `lessons/21/src/engine/gpu/device.rs`

```rust
--8<-- "lessons/22/src/engine/gpu/device.rs:step-1a"
```

Added after the line `}` in `lessons/21/src/engine/gpu/device.rs`

```rust
--8<-- "lessons/22/src/engine/gpu/device.rs:step-1b"
```
...
Every file at this point: `lessons/22/`.

## Next

[23 · Create, trim, extend and explode](23-geometry-commands.md)
...
[![Full viewer result for lesson 22](screenshots/current-empty.png)](screenshots/current-empty.png)
