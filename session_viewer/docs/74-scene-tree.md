# 74 Scene tree — the documents, as a panel
...
> side panel, an eye icon per row driving 50's visibility, names instead of guids. Two disciplines
...
<svg viewBox="0 0 680 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="the docs each contribute a top-level row; each session tree flattens to visible rows only; each row shows an eye toggle and a name; toggles call the same scene verbs as the CLI" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g transform="translate(280,20)">
    ...
    <text x="0" y="54" fill="#888">eye → scene.hidden + flags (50's verbs — ONE authority)</text>
    <text x="0" y="74" fill="#888">click → scene.selected (49's verbs)</text>
...
src/state.rs     # apply collected intents (toggle/select) after the closure — 51's rule
...
pub fn flatten(scene: &Scene, expanded: &HashSet<String>) -> Vec<Row> {
    ...
    for (d, doc) in scene.docs.iter().enumerate() {
        ...
        // objects the tree doesn't parent (68's nurbs collections) → rows under the doc
...
```rust
    ...
    /// Renderable guids of doc `d` with no tree node — collection-only citizens (68) become
...
`remove_object` maintains (55), so deleted objects leave the panel automatically.)

## Step 2 — the rows, virtualized: `src/ui/tree.rs`

`TreeIntent` is the collect-then-apply buffer (51's rule) the panel fills and `State` drains — add
...
the control out of reach), and the selected row's style is **white background, black text** — 51's
...
`TreeIntent { toggled, expand_toggled, clicked }` collects inside the closure (51's rule); `State`
...
- Click an eye on a LAYER → the whole layer blinks out in the viewport *and* stops picking (50's
...
- Click a name → viewport selection + gumball follow (49/56). Shift+click adds. The selected row
  reads black-on-white.
- Delete an object (55) → its row is gone next frame; undo → row returns. The panel was never told —
  it renders each `session.tree`, which the Commands already maintain.

## Recap

```
Ch 73: Phase 11 closed — the look, fast.
Ch 74: THE TREE. Top level = one row per Doc (manifest name + object count); each doc's
       ...
       ((doc, name) tuples); State drains it through 50's visibility and 49's selection verbs — one
...
`75-tree-viewport.md` — the two views converge: picking in the viewport reveals and scrolls the tree
