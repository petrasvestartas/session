# 73 Selection outline + AA polish — the look, without the tax
...
> the mask renders **only when dirty** (70's flag, scoped), the search becomes **separable** (two
...
<svg viewBox="0 0 680 140" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="selected objects render once into a small mask when selection changes; a horizontal then vertical distance pass expands it; composite draws a crisp ramp where distance is under the width" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <text x="340" y="112" fill="#888" text-anchor="middle">nothing selected → all three passes skip; static scene → nothing renders at all (70)</text>
...
const FLAG_SELECTED: u32 = 1u;   // bit 0 (49)
...
Add the same 6-line fullscreen-triangle `vs_main` the GTAO/composite shaders carry (69's trick) to
...
71's AO texture; reuse the fullscreen `samp` sampler the pass already binds), plus the ring width:

```wgsl
// find the composite binding block (71's AO texture) → add after it:
...
The plumbing is 71's post-process pattern, one more time. **Targets** — beside 71's half-res AO pair
...
frame entry point, between 71's blur pass and the composite (the composite samples
`outline_dist_b`, so it must be final by then):

```rust
        // ---- outline (73): mask → separable distance ×2 — only when it can matter ----
...
built once in `Gpu::new`, the same `build_*_pipeline` + layout/bind-group moves as 71's gtao/blur
...
- Select an object → a crisp amber ring, ~3 px, smooth on diagonals (coverage + ramp), hugging the
  silhouette — including through holes (68's trimmed surface outlines its hole rim too: the mask is
  geometry, not a bounding shape).
- Watch `drawn/s` (70) and frame time: selection sitting still = **no outline passes re-run**;
...
```
Ch 72: arctic GI — the look, cheap.
Ch 73: OUTLINE, structural fixes only. Mask = selected instances rendered white (FLAG_SELECTED
       ...
       only. Gated: no selection → no passes; unchanged frame → nothing at all (70). FXAA retired —
...
`74-scene-tree.md` — Phase 12: scene management UI. The Session's tree appears in a panel —
virtualized so a thousand rows scroll smoothly, with an eye icon driving 50's visibility per object.
