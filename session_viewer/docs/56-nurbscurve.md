# 56 NurbsCurve — what already draws, made honest and reusable
...
points in an EDIT mode; that is lesson 98's job (`F10`, on the selected curve only), and the
...
`57-nurbssurface.md` — surfaces already draw too (`s.mesh()` straight into `push_mesh`) — and
