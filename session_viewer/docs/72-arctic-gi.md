# 72 Arctic + cheap GI — a better default look for the same money
...
<svg viewBox="0 0 680 130" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="the ao and bent normal from 67 feed three effects in the composite: sky visibility, multibounce, and key light micro shadowing" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <text x="85" y="57" fill="#d7dae0" text-anchor="middle">71's outputs</text>
...
src/state.rs / ui            # B toggle + settings checkbox (51)
```
...
67 stored the bent normal as `bent*0.5+0.5` in the AO target's `gba` channels (72's `RGBA16Float`: `r`=ao,
`gba`=bent), so decoding is one line — add it beside `multi_bounce` at the top of `composite.wgsl`:

```wgsl
// decode 71's bent normal (stored bent*0.5+0.5 in gba)
...
```wgsl
    let bent = decode_bent(ao_tex_sample.gba);                 // ao_tex_sample = 71's upsampled texel (r=ao)
    ...
    var ambient = scene_rgb * sky_vis;                          // scene_rgb = 71's lit color; there is no separate ambient term
```
...
soft, background near-white (69's ground already matches). One uniform flag branches the composite;
...
value (71's, static) scaled to one LSB:
...
- The sky gradient (69's fade region) shows **no banding** stripes (Step 4's dither; comment it out
...
```
Ch 71: GTAO — ao + bent normal, ~12 reads/px.
Ch 72: THE LOOK, same money. Three effects, all in the composite, ZERO new fetches: (1) sky
...
`73-outline-aa.md` — the last archive-look piece: the crisp selection outline, rebuilt without the
per-frame tax — mask passes render only when dirty (70), the 81-tap box search becomes two separable
