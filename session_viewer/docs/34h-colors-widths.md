...
with the same line: `final = row_color × instances[id].color`. Selection (lesson 69) will recolor an
...
Ch 34h: RESOLVE COLORS/WIDTHS ONCE, CPU-SIDE. Row color = the user's color (precedence:
        ...
        multiplied in all the shaders (selection's channel, lesson 69). Width = multiplier in the
