...
```rust
    ...
    /// CVs written back HOMOGENEOUS with their original weights — 78's rule, unchanged. And 78's
...
`greville_cache.invalidate(guid)` everywhere 78's release bookkeeping calls `tess_cache.remove`,
and drop entries on delete/undo (56) and reconcile (43b), or dead guids accumulate for the whole
...
- Edit-point mode on 65's curve: amber points sit **on** the line. Drag one — the curve follows so
  ...
  *placed* sheet — the point still tracks the cursor (78's world→local conversion runs here too).
