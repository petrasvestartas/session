...
projective matrix. Lesson 53's screen-ray unprojection through `inverse(view_proj)` was broken by it
...
the C++ variant + ports is the follow-up; until then the viewer's `all_objects()` (lesson 71) still
...
Reconcile (46) and save-gating (44) need "did this object change?". `{:?}` is unusable — `Mesh`
...
has **no `jsondump`**, so 40 fingerprints it via `mesh() + xform` — expensive and lossy.
...
pre-cached tessellations"). The viewer built a guid-keyed cache (lessons 68/63); every other consumer
...
or write STEP at all (lesson 86 wires the dispatch arm to warn loudly instead). This is the largest
...
functions now delegating, ×3 languages + a "String Roundtrip" minitest ×3. Enabled lesson 86.
...
Enabled lesson 92's copy/array/Alt-drag. Bonus find: C++ `Mesh`'s hand-written copy
...
- **`OBB::from_nurbssurface` is trim-blind** — fine for untrimmed, but `NurbsSurfaceTrimmed` boxes
  must come from the tessellation (lesson 71 does); a trimmed-aware overload would centralize it.
...
Each entry traces to a concrete tutorial workaround: 36 (`world_obb`), 40 (`content_hash`), 41
