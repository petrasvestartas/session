# 56 Delete + undo/redo — the pattern everything stands on
...
Three new arms in `dispatch` (add `"delete"`, `"undo"`, `"redo"` to 55's `VERBS`; aliases
...
`match event.logical_key.as_ref()`, beside the Escape arm (53):
...
```
Ch 55: history/autocomplete — CLI ergonomics.
Ch 56: UNDO. trait Command { apply / revert / label } + History { done, undone } — the archive's
...
`57-gumball-geometry.md` — Phase 9: transform & draw. The 3-axis gizmo appears at the selection
