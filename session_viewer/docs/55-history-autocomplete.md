# 55 History & autocomplete — the CLI grows muscle memory
...
> This is a small lesson by design — three quality-of-life features on 53's CLI, no new architecture
...
53's `match` arms embedded the aliases in patterns; completion needs them *as data*. One pair of
...
and at the top of `dispatch`, replace 53's `let verb = parts.next().unwrap_or("");` with:
...
```rust
    pub cli_history: Vec<String>,        // ← ADD to State (init empty in State::new)
                                         //   NB: named cli_history, not history — lesson 56 adds
...
Now replace 53's `cli_panel` wholesale with the version below. Note the `history: &[String]`
...
Inside 53's `build_ui` closure (where `cli_panel` is already called and its return stashed in
...
`56-delete-undo.md` — the first *destructive* mutation, and with it the pattern the whole editor
