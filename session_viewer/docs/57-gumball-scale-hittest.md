# 57 Gumball II — constant size + pickable handles
...
`vp_h` lines are 48's pick-site trio verbatim, and the camera fields (`unit`, `orientation`,
...
```rust
    ...
    /// distance (the archive's orbit-breathing bug). Mirrors 48's world_per_pixel per projection.
    fn gumball_scale(&self, origin: [f32; 3]) -> f32 {
        ...
        let proj_y  = 1.0 / (30.0_f64).to_radians().tan() * unit;           // 48's trio, verbatim
...
(`use session_rust::Vector;` if `state.rs` doesn't import it yet.) In `refresh_gumball` (56), find
...
```rust
        if self.gb.is_some() { self.refresh_gumball(); }   // depth changed with the camera (57)
```
...
The build already tags every row with its `HandleKind` (56). Hit-testing is: distance from the pick
...
```rust
/// Distance-based handle pick. `tol` in world units (48's world_per_pixel × ~20 px — handles are
...
The two distance helpers are the classic closest-approach formulas, in f64 against 45's
...
`gumball.rs` (below 56's `use crate::engine::gpu::…` line). ~460 primitives, run per mouse event,
...
The input pecking order grows one level. From top: egui (51) → Get-loop (52) → **gumball** → scene
picking (42–45). `ray` here is 46's `engine::pick::screen_to_world_ray(&vp, &origin, self.cursor,
viewport)?` result (built early, so it's in scope). `proj_y`/`ortho_h`/`vp_h` are 48's three pick-site
...
This is the cursor-move handler, **not** the press handler — 48's `proj_y`/`ortho_h`/`vp_h` locals
...
45's `(f64, f64)`) and `gb_hovered: Option<HandleKind>` to `struct State`, and initialize **both to
`None`** in `State::new`. `state.rs` needs `use crate::engine::gumball::HandleKind;` for the field
types.

The tint itself is three edits. In `gumball.rs`, find 56's `build` signature:
...
Finally, in `refresh_gumball` (56), the `build(…)` call gains the new argument:
...
```
Ch 56: the widget exists — geometry, overlay pass, centroid anchor.
Ch 57: USABLE. Scale = SCREEN_PX(140) · world_per_px(depth) / ARC_RADIUS with depth = VIEW-SPACE Z
...
`58-gumball-translate.md` — the first real transform: press an arrow, drag along its axis, watch the
