# 84 Copy, duplicate, array — daily-use editing, nearly free
...
> geometry type), **doc-aware insertion** (61's `add_object` rail, wrapped in one `AddGeometry` so
...
<svg viewBox="0 0 680 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="copy duplicates the selection minting fresh guids, inserts the batch into each source doc as one AddGeometry command, then places every copy with apply world delta" style="max-width:100%;height:auto;font:11px ui-monospace,monospace">
  ...
  <g fill="#d7dae0" text-anchor="middle">
    ...
    <text x="417" y="47">ONE AddGeometry (61)</text><text x="417" y="59" fill="#666" font-size="9">insert batch → one undo</text>
  ...
  <text x="340" y="100" fill="#888" text-anchor="middle">array = the same pipeline in a loop; Alt+gumball = the same pipeline on 58's release</text>
...
`Geometry` variants hold **`Rc<T>`** (55). So `lookup.get(guid).cloned()` clones the *handle*: the
...
- **its doc** — `guid_to_row` + `doc_of_row` (40). Copies go into the **source object's doc**, so
...
`impl AddGeometry` block (61) and add this beside `one` (`RemovedObj` is 55's snapshot shape —
...
```rust
    ...
    /// `one`, plural. Each snapshot inserts through 61's add_object path into ITS OWN doc.
...
```rust
    /// Doc-resolved duplicates of the selection, in 55's snapshot shape — ready for ONE
...
their rows resolve through `guid_to_row`, and each gets 58's commit primitive plus the live GPU
poke:

```rust
    /// Post-insert placement for a batch of copies: 58's apply_world_delta per new row
...
`CopyCmd` is 61's `LineTool` shape exactly — a struct with `from: Option<Point>`, the first click
stashes it, the second runs the batch: duplicate → ONE `AddGeometry` (the insert routes each
snapshot through 61's `add_object` into its own doc and `set_xform`s the carried local — the copy
...
(Snap (63) applies to both picks automatically, so copy-from-corner-to-corner is *exact*.)
...
One branch on 58's **release**, not its press: the drag runs exactly as 54 built it (live,
...
```rust
        // 58's release handler, first lines:
        if let Some(ctx) = self.gb_drag.take() {
            ...
            // …the normal commit continues unchanged (58's apply_world_delta + TransformObjects)…
        }
```

The subtlety that makes this clean: 58's live drag **never mutates the kernel objects** — so
...
- **Alt+drag** the X arrow → the original stays put, release → a copy at the drop point, one undo
  step. Without Alt → 58's normal move, unchanged.
...
```
Ch 83: files in and out.
Ch 84: DUPLICATION = three rails composed: duplicate() + AddGeometry + apply_world_delta. The trap:
       ...
       AddGeometry::of_snapshots (inserts via 61's add_object into the SOURCE doc; undo removes the
       batch) → place_copies: apply_world_delta per new row (rows exist post-insert, via
       guid_to_row) + the live poke. array = the loop form, still one Command. Alt+gumball = 58's
       drag untouched until RELEASE: originals snap home (live path never touched kernel objects —
       58's Esc discipline, second customer), copies take the final delta.
```

Edited: `app/history/add.rs` (`AddGeometry::of_snapshots`), `app/scene.rs` (`clone_selection`),
`app/commands.rs` (`copy`, `array`), `state.rs` (`place_copies`, Alt branch on 58's release).

## Next

`85-layers.md` — organization users expect: layers — which the PDF importer already built as tree
