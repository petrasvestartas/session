# 59 Gumball IV — rotate and scale
...
For `RotateX`: `n = X`, `a_dir = Y`, `b_dir = Z` — the same `(i+1)%3 / (i+2)%3` pairing 56's arc
...
58's code untouched. That's the skeleton paying off. (`state.rs` gains `Line` in its
...
**Fix 1 — preserve the sign.** The axis-scale spheres sit on the **negative** axis (56). The naive
...
Axis scale reuses 58's `closest_param_on_axis` for `t`; uniform scale measures the ray's hit
...
**3a. The stash.** Add `a0: f64,` and `d0: f64,` to 58's `DragCtx` struct. In `begin_drag` (58),
...
**3b. The dispatch.** In 58's mouse-move handler, find the whole
        ...
        if let (Some(ctx), Some(ray)) = (&self.gb_drag, self.cursor_ray()) {
            ...
            if let Some(delta) = delta {
                for (_, row, base) in &ctx.base_models {
                    self.gpu.set_live_model(*row, &(&delta * base));          // 58's live path
                }
...
(The `if let Some(delta) = live_delta { … }` stash + `refresh_gumball_at` lines after it are 58's,
unchanged.)

The commit path does not change **at all** — the release still hands the final delta to 58's
...
**world-space**, and 58's helper conjugates it by the doc's `place` internally — for translate that
...
```
Ch 58: translate — the drag skeleton (defer, live matrix, Command commit).
Ch 59: ROTATE + SCALE = two delta functions on that skeleton. Rotate: ray ∩ arc plane (denom guard
...
`60-gumball-numeric.md` — the click that 58's threshold deliberately left free: click a handle
