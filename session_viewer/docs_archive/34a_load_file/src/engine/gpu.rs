...
impl Gpu {
    ...
    pub async fn new(window: std::sync::Arc<winit::window::Window>) -> anyhow::Result<Self> {
        ...
        let objects: Vec<(Mesh, Xform, [f32; 4])> = vec![
            ...
            (Mesh::create_box(600.0, 600.0, 600.0),      Xform::translation(1.0e7, 0.0, 0.0), [1.0, 1.0, 1.0, 1.0]),
        ...
        let mut objects_base: Vec<(Xform, [f32; 4])> = Vec::with_capacity(objects.len()); // keep a copy while building the arena
        ...
        for (ri, (mesh, model, color)) in objects.into_iter().enumerate(){
            objects_base.push((model.duplicate(), color));
            ...
            // Endpoints - one pshere glyph each
            // instance_id = this pobject's row (ri)
            for vk in mesh.vertices(){
                let p = mesh.vertex_point(vk).unwrap();
                glyphs.push(GlyphPoint { 
                    center: p.to_f32(), 
                    radius: 0.0, // screen-constanc px 
        ...
        // Pointcloud load from a file
        let bunny = session_rust::read_xyz_from_str(include_str!("../../../session_data/bunny_rgb.xyz"));

        let cloud_ri = instances.len() as u32; // the cloud's own isntance row
        instances.push(Instance {
            model: (Xform::translation(0.0, 0.0, 900.0) * Xform::scale_xyz(4000.0, 4000.0, 4000.0)).to_f32(), 
            color: [0.0; 4], 
            flags: 0, 
            _pad: [0; 3], 
        });

        // use the file's per-point color when it carried one (color_count matches), else a fallback
        let has_color = bunny.color_count() == bunny.point_count();
