...
/// sweep line weight without a rebuild; unset (and always on wasm) it is 1.0.
///
/// 1.0, not the 2.0 this used to be: a pen measured in PIXELS keeps its weight while the
/// viewport shrinks, so a model embedded in a page at a third of the screen kept full-fat 2 px
/// ink over a third of the geometry and read as a blob of lines.
fn line_thickness_px() -> f32 {
    std::env::var("VIEWER_THICKNESS").ok().and_then(|v| v.parse().ok()).unwrap_or(1.0)
}
