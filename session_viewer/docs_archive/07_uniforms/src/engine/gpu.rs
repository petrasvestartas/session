...
pub struct Gpu {
    ...
    pub aspect_buffer: wgpu::Buffer,         // Aspect ratio
    pub aspect_bind_group: wgpu::BindGroup,  // Aspect ratio
...
impl Gpu {
    ...
    pub async fn new(window: std::sync::Arc<winit::window::Window>) -> anyhow::Result<Self> {
        ...
        Ok(Self { surface, device, queue, config, pipelines, aspect_buffer, aspect_bind_group, vertex_buffer, num_vertices, time:0.0, time_buffer, time_bind_group })
    }
