...
DESCRIPTIONS_BY_LESSON = {
    ...
    "29-instancing.md": [
        "Chapter twenty-eight read eight draws for three objects — background, grid, three meshes, three edge-sets. Chapter twenty-nine collapses the three mesh draws into one: three draws for a hundred objects.",
        "We touch four files: gpu rs, for the instance row and the storage buffer; the triangle shader, to read the instance table; and the two pipeline files, build rs and mod rs, to give the triangle pipeline a third bind group.",
        "One row per copy: a model matrix, sixteen floats straight from Xform to-f-thirty-two; a color, four floats; a flags word reserved for later, selection and culling; and twelve bytes of padding, so the row is exactly ninety-six bytes, the stride a storage array needs. Bytemuck checks the size against the shader struct for us.",
        "In the shader, the instance table is declared at group two, binding zero — a read-only storage array of those rows. The vertex shader reads its own row by the built-in instance index, applies the row's model matrix to take the mesh's local vertex to world, replaces the per-vertex color with the row's color, and rotates the normal by the model, dropping the translation. One mesh, but each copy placed and tinted by its row.",
        "The triangle pipeline's layout must now list a third bind-group layout so group two is legal. We add an instance-layout parameter to build-triangle-pipeline and append it to the bind group layouts.",
        "We thread that instance layout through Pipelines new — only the triangle pipeline needs it; the grid, edges, and background pipelines are unchanged.",
        "We replace the three demo meshes with one source dodecahedron, uploaded once, and a hundred instance rows: a centred ten by ten grid of translations, each tinted by its grid position — red rising across, green rising down — so the win is obvious at a glance.",
        "We upload the hundred rows once as a storage buffer, and build its bind group: vertex-visible and read-only. This is the table the shader indexes by instance index.",
        "We build the pipelines after the instance layout exists, passing it in, so the triangle pipeline's group two is wired to the table.",
        "The mesh loop collapses to a single instanced draw. We bind the camera, the time, and the instance table at groups zero, one, and two; set the one source mesh's vertex and index buffers; and draw indexed over the range zero to one hundred — a hundred copies from one call.",
        "We report the instance count as the object count, so the perf line tells the real story, then submit and present as before.",
        "To run it, change into session viewer and run trunk serve, then open local host, port eight seven seven zero.",
        "The perf line now reads about sixty frames per second, sixteen point six seven milliseconds, three draws, one hundred objects.",
        "To recap: chapter twenty-nine draws the same mesh many times from one call. Per-copy data — a model matrix, a color, and reserved flags — lives in a storage buffer indexed by the built-in instance index, the G P U-driven path. A ten by ten dodecahedron field draws in one indexed call: three draws for a hundred objects. Edges are parked until lesson thirty-one.",
    ],
    "30-batching.md": [
        "Instancing and batching are the two ways to cut draws, and they trade off oppositely. Instancing keeps one mesh's vertices in the G P U and replays them N times — cheap memory, but the geometry must be identical. Batching concatenates many different meshes' vertices into one arena — arbitrary geometry, each vertex stored once.",
        "Chapter twenty-nine drew a hundred identical copies in three draws. Chapter thirty draws five distinct meshes, also in three draws — background, grid, and one arena.",
        "We touch three files: gpu rs, to build the arena and draw it; the triangle shader, to read a per-vertex row id; and build rs, to give the triangle pipeline a second vertex buffer for those ids. The group-two instance table from chapter twenty-nine is reused unchanged.",
        "In the shader's vertex input, we add a fourth field at location three — a u-thirty-two called inst-id, the instance row this vertex belongs to.",
        "In the vertex shader, we drop the built-in instance index — a single batched draw has only one instance, so it's always zero and useless. Instead we read the row from the vertex's own inst-id. Everything below is unchanged.",
        "In build rs, we add the layout for the id buffer: one attribute, a u-thirty-two at location three, stride four bytes, advancing per vertex like position.",
        "In the triangle pipeline only, we append that id layout as a second vertex buffer, slot one, next to the render-vertex layout. The grid, edges, and background pipelines are left alone.",
        "In new, we delete the single source dodecahedron and the whole ten-by-ten instancing loop — the arena replaces them.",
        "Only box and dodecahedron are mesh factories; sphere, cylinder, and torus are Brep factories, so we call dot mesh on those. We add Brep to the import and drop Color, which the arena no longer uses.",
        "In its place, a scene of five different meshes — a box, a dodecahedron, a sphere, a cylinder, and a torus — each with its own placement and color. We fold them into three parallel arrays: all their vertices concatenated, one row id per vertex, and one shared index list. Each mesh also gets one instance row. For each mesh we record where its vertices begin, flatten it to f-thirty-two, push its vertices tagged with the row id, and append its indices offset by that base — so a single draw can reach every mesh.",
        "We upload the three arena buffers: the vertices, the per-vertex row ids, and the shared index buffer.",
        "In the Gpu struct, the single mesh field becomes four arena fields — the vertex buffer, the id buffer, the index buffer, and the index count. The instance table and its bind group stay.",
        "In the returned struct, we swap the mesh field for the four arena fields.",
        "In clear, this is the old per-mesh draw we're replacing — it fetched one mesh's cached buffers and drew a hundred instances of it.",
        "We replace it with the arena binds and a single whole-scene draw: bind the vertices at slot zero, the row ids at slot one, the index buffer, and draw the entire arena as one instance — one call for the whole scene.",
        "To run it, change into session viewer and run trunk serve, then open local host, port eight seven seven zero.",
        "The perf line reads about sixty frames per second, sixteen point six seven milliseconds, three draws, five objects.",
        "The single-draw arena stores every object's vertices once, so a hundred copies would duplicate the vertices a hundred times. Instancing cures that, and the two combine, because both read the same instance table. The trick is to stop baking the base vertex into the indices and instead record, per distinct mesh, its slice of the index buffer, where its vertices start, and which rows point at it.",
        "Then the mesh pass is a short loop: bind the buffers once, and issue one instanced draw per distinct mesh, replaying its slice over however many rows share it. An individual mesh has one row; an instanced mesh, like every edge of the scene, has many, with the vertices stored once.",
        "To recap: chapter twenty-nine replayed one mesh many times by the built-in instance index. Chapter thirty concatenates many different meshes into one vertex and index arena, drawn in a single indexed call; the vertex-to-row link moves to a per-vertex u-thirty-two id in a second buffer, and indices are folded arena-global. Five distinct solids, three draws. Combine the two — a per-mesh descriptor of index range, base vertex, and rows — and one instanced draw per distinct mesh handles individual and repeated geometry alike. That's the path lesson thirty-one takes.",
    ],
    "31-edges.md": [
        "Instancing in chapter twenty-nine used one template mesh and N instance rows, drawn in one call by the built-in instance index. Chapter thirty-one uses one cylinder template and one row per edge — the built-in instance index now selects a segment instead of a copy, still one draw.",
        "Chapter thirty was three draws for five meshes. Chapter thirty-one adds a fourth: background, grid, the arena, and one cylinder call carrying every edge in the scene.",
        "We touch five files: a new cylinder shader; build rs, for the cylinder pipeline and the template's position-only layout; mod rs, to add the cylinder pipeline; gpu rs, for the segment row, the buffers, and the one draw; and in the kernel, mesh rs, for edges-with-colors, which walks edges in line-color order. The group-two instance table from chapter thirty is reused.",
        "One row per edge, the cylinder segment: two endpoints in the mesh's local space, a radius sentinel — zero means screen-constant pixels, positive means a world-millimetre override — the instance id of the object the edge belongs to, and a per-edge color. It's forty-eight bytes, three sixteen-byte rows with no padding. The color is inline so a single mesh can carry distinct edge colors.",
        "In the kernel, a new method, edges-with-colors, replays the face traversal so the Nth unique edge lines up with the Nth stored line color. The existing edges method sorts endpoints and so does not align with the color list; this walk does. It returns each edge as two vertex indices plus its color — the permanent per-edge-color source, so crease, naked, and selection edges can each carry their own color.",
        "The cylinder shader. It reads the camera matrix, a line uniform for thickness, the reused group-two instance table for each object's transform, and a group-three storage table of segments. The vertex shader takes a position-only template vertex, reads its segment by the built-in instance index, transforms the endpoints to world through the object's model, aligns the template's plus-Z to the edge direction by building an orthonormal frame on the fly, and expands the ring by a screen-constant radius — a world radius that projects to the wanted pixel width, computed from the centre point's clip w so radius and depth don't chase each other. The fragment shader just returns the edge color.",
        "In build rs, the template's vertex layout: just a three-component position at location zero, stride twelve bytes — the unit cylinder carries nothing else.",
        "Build-cylinder-pipeline. It lists four bind groups — the camera matrix, the line uniform, the reused instances, and the new segments storage — and draws solid triangle-list tubes with depth-write on. Because the tubes physically protrude from the faces, they need no depth bias to sit on top, unlike the flat lines of lesson twenty-three. It reads reverse-Z, the Greater compare, and four-sample anti-aliasing, like the other pipelines.",
        "In mod rs, the Pipelines struct gains a cylinder field, after edges and before background.",
        "Pipelines new gains two new layout parameters — the line uniform and the segments — and builds the cylinder pipeline alongside the others. The grid, edges, triangle, and background pipelines are unchanged.",
        "In gpu rs, we declare a segments vector beside the arena's vertices, ids, and indices.",
        "Inside the arena loop, after appending the mesh's indices, we extract its edges: for each edge and color from edges-with-colors, we look up the two endpoints, keep them local, and push one cylinder segment with the radius sentinel zero and this object's row as the instance id. Every edge of every mesh lands in one flat segment list.",
        "The unit-cylinder template: a free function that builds a cylinder along plus-Z, radius one, z from zero to one, with side quads and two cap fans. The sides count is the perf knob — twelve gives smooth tubes, six to eight is plenty at one or two pixels. The shader rescales the x-y by the screen radius and maps z along the edge, so this template is built once.",
        "We upload the template's vertices and indices, the segment table as a vertex-visible read-only storage buffer with its bind group, and the line uniform — thickness in pixels — as a uniform buffer with its bind group. Three resources: one template mesh, one row per edge, one thickness value.",
        "The line uniform struct: thickness in pixels, a vertical projection scale, an ortho half-height that's zero in perspective, and the framebuffer height — sixteen bytes, one vector, no padding.",
        "We pass the two new layouts, the line uniform and the segments, to Pipelines new.",
        "In the Gpu struct, we add the template buffers and index count, the segment buffer, its bind group and count, and the line buffer and its bind group.",
        "And we name them all in the returned struct.",
        "In clear, right after the arena draw and still inside the render pass, the cylinder pass: bind the camera, the line uniform, the reused instances, and the segments; set the template's vertex and index buffers; and one instanced indexed draw — one template, N edges. One call for the whole scene's linework.",
        "The tube radius depends on the viewport height and the projection, so we rewrite the line uniform each frame, beside the existing time and camera writes: thickness two pixels, the projection scale from the sixty-degree field of view folded with the millimetre-to-metre unit scale, ortho half-height zero for perspective, and the current framebuffer height.",
        "To run it, change into session viewer and run trunk serve, then open local host, port eight seven seven zero.",
        "The perf line reads about sixty frames per second, sixteen point six seven milliseconds, four draws, five objects. Four draws: background, grid, the arena, and one cylinder call carrying every edge. Zoom hard and the tubes hold their pixel width; bump thickness to eight and they fatten with zero re-upload — one uniform write.",
        "To recap: chapter thirty-one brings edges back as cylinders. One unit-cylinder template plus one segment row per edge — local endpoints, a radius sentinel, an instance id into the object's model and flags, and an inline per-edge color — in a flat storage table. One indexed draw for the whole scene's linework. The vertex shader aligns plus-Z to the edge per instance, with no per-segment matrix, and expands the radius to a screen-constant pixel width through the line camera uniform. Per-edge colors come from the kernel's new edges-with-colors, walked in line-color order. The lesson twenty-three line pipeline survives for overlays only. Four draws for five objects, flat as the scene grows, and thickness changes are a single uniform write.",
    ],
}
