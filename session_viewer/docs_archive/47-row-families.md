# 47 Row families — one file per row format

> Third refactor lesson. Start from the end of lesson 46. Pixels stay identical on every
...
<svg viewBox="0 0 720 300" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="one row family: CPU rows append into a GrowBuf, a bind group exposes it, the family's pipelines draw it and return a draw count; instantiated for arena, segments and glyphs" style="max-width:100%;height:auto;font:12px ui-monospace,monospace">
  ...
  <text x="360" y="16" fill="#888" font-size="11" text-anchor="middle">one row family — engine/gpu/&lt;family&gt;.rs (lesson 47)</text>
  <g fill="none">
...
pub enum LineStyle {
    ...
    Tubes,
    /// A camera-facing quad per edge: 6 list vertices, the flat lane's own shader. Cheaper, and it
...
impl SegmentLane {
    ...
    pub fn draw_pipes(&self, pass: &mut wgpu::RenderPass<'_>, p: &Pipelines, b: &Binds, style: LineStyle) -> u32 {
        ...
        match style {
            ...
            LineStyle::Flat => {
                ...
                pass.draw(0..6 * self.pipes.len(), 0..1); // 6 verts/segment, see ribbon.wgsl vs_main
    ...
    pub fn draw_ribbons(&self, pass: &mut wgpu::RenderPass<'_>, p: &Pipelines, b: &Binds) -> u32 {
        ...
        pass.draw(0..6 * self.ribbons.len(), 0..1);
        1
    }
    ...
    pub fn draw_ribbon_depth(&self, pass: &mut wgpu::RenderPass<'_>, p: &Pipelines, b: &Binds) -> u32 {
        ...
        pass.set_bind_group(2, b.instances, &[]);
        pass.set_bind_group(3, &self.ribbon_group, &[]);
        pass.draw(0..6 * self.ribbons.len(), 0..1); // 6 verts/segment, see ribbon.wgsl vs_main
        1
    }
...
//! segments, glyphs - one file each, one shader row format each), and, until lesson 48, the
...
use crate::engine::pipelines::{Layouts, Pipelines, Target, SPLAT_COLOR_FORMAT};
use crate::math::Aabb;
use session_rust::Point;
...
pub struct Gpu {
    ...
    frame_no: u64,     // frames presented, for the `?perf=1` line
    last_frame_t: f64, // when the previous one was presented, ms
    pub bounds: Aabb,
}
...
impl Gpu {
    ...
    async fn build(
        ...
        // the point pass - the cloud's own targets, record tables, groups
    ...
    pub fn set_scene(&mut self, up: &Upload) {
        ...
        // file and `reset_arena` (a clear) starts it over. An empty upload (the State boots
        // before any file) is the inverted box and widens nothing.
    ...
    /// Splat group 1 for one lane: its point buffers. The depth and colour the lane draws into
    ...
    pub fn cloud_pos(&mut self, pos: &[f32]) {
        ...
        self.ctx.queue.submit([]);
        self.splat_state = None; // new points - the point targets are stale
    }
    ...
    /// Reconfigure the surface and recreate the depth + MSAA targets AND the cloud's own
    /// targets for a new canvas size.
    ...
    pub fn encode_frame(
        ...
        let b = Binds { mvp: &self.frame.mvp_group, line: &self.frame.line_group, instances: &self.objects.group };
        {
            ...
            // VIEWER_SKIP (native only) leaves a named lane out; always on in the browser.
            let on = |lane: &str| !self.view.skip(lane);
            ...
            if self.view.show_mesh_edges && on("pipes") {
                draws += self.segments.draw_pipes(&mut pass, &self.pipelines, &b, self.view.line_style);
            }

            // The cloud lane, drawn with the solids: the point pass already resolved every
            // cloud into its own depth/colour targets, so the whole lane is one fullscreen
            // triangle that composites them - depth-writing via frag_depth, so points and
            // solids occlude each other exactly.
            if self.splat_total > 0 && on("splat") {
            ...
            if self.view.show_mesh_edges && self.view.markers && on("spheres") {
            ...
            if INK_DEPTH_PREPASS && self.view.show_lines && on("ribbon_depth") {
                draws += self.segments.draw_ribbon_depth(&mut pass, &self.pipelines, &b);
            }
            if INK_DEPTH_PREPASS && self.view.show_points && on("glyph_depth") {
                draws += self.glyphs.draw_dot_depth(&mut pass, &self.pipelines, &b);
            }

            if self.view.show_lines && on("ribbon") {
            ...
            if self.view.show_points && on("glyph") {
...
Lesson [47](48-point-lanes.md) — the point lanes and the frame list: `cloud.rs`, `stream.rs`,
