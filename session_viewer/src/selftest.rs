...
/// Verify retained source-edge identities after production upload, using one native device.
...
/// BRep pipes are contiguous; each must resolve to its original non-degenerate edge.
