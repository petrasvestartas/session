...
/// Stress-test grid: replicate the loaded scene into an N×N tile grid (1 = off).
/// Tiles are extra INSTANCE/table rows with offset placements — the source geometry is parsed once.
...
pub struct SceneTables {
    ...
    min: [f32; 3], // raw table extent (mesh verts are local — fine while fixture
    max: [f32; 3], // xforms are near-identity; exact per-object bounds is lesson 36)
}
...
impl Gpu {
    /// Walk one parsed session into GPU-ready tables. Called per file DURING loading so the
    /// session drops immediately after; `Gpu::new` then only sees the compact tables.
    pub fn walk_session(session: &session_rust::Session) -> SceneTables {
        let mut t = SceneTables {
            verts: Vec::new(), vids: Vec::new(), idx: Vec::new(),
            segments: Vec::new(), glyphs: Vec::new(),
            objects: Vec::with_capacity(session.lookup.len()),
            min: [f32::INFINITY; 3], max: [f32::NEG_INFINITY; 3],
        };
        ...
        // instance model. `ri` is the row in t.objects, not the lookup index - skipped
        ...
        for geom in session.lookup.values() {
            ...
            match geom{
                Geometry::Mesh(m) => {
                    ...
                    push_mesh(m, ri, &mut t.verts, &mut t.vids, &mut t.idx, &mut t.segments, &mut t.glyphs);
                }
                Geometry::BRep(b) => {
                    let bm = b.mesh();
                    t.objects.push((b.xform.clone(), b.surfacecolor.to_f32()));
                    push_mesh(&bm, ri, &mut t.verts, &mut t.vids, &mut t.idx, &mut t.segments, &mut t.glyphs);
                }
                Geometry::Line(l) => {
    ...
    pub async fn new(
        ...
        // ── Grid layout: cells CYCLE through the loaded files (different drawing per cell);
        // STRESS_GRID² sets a floor on the cell count for load testing ──────────────────────
        let cells = ((STRESS_GRID * STRESS_GRID) as usize).max(files.len());
        let cols = (cells as f64).sqrt().ceil() as usize;
        let mut cell_w = 0f32;
        let mut cell_h = 0f32;
        for t in files { if t.min[0].is_finite() {
            cell_w = cell_w.max(t.max[0] - t.min[0]);
            cell_h = cell_h.max(t.max[1] - t.min[1]);
        } }
        let dx = (cell_w * 1.05) as f64; // 5% gutter between cells
        let dy = (cell_h * 1.05) as f64;
        ...
        if !files.is_empty() {
            for cell in 0..cells {
                ...
                let ox = (cell % cols) as f64 * dx;
                let oy = (cell / cols) as f64 * dy;
                let off = Xform::translation(ox, oy, 0.0);
                ...
                for (i, v) in t.verts.iter().enumerate() {
                    verts.push(*v);
                    vids.push(t.vids[i] + ri0);
                }
                for id in &t.idx {
                    idx.push(*id + vbase);
                }
                // Scene bounds INCLUDE the cell offset — F fits the whole grid.
                if t.min[0].is_finite() {
                    scene_min[0] = scene_min[0].min(t.min[0] + ox as f32);
                    scene_min[1] = scene_min[1].min(t.min[1] + oy as f32);
                    scene_min[2] = scene_min[2].min(t.min[2]);
                    scene_max[0] = scene_max[0].max(t.max[0] + ox as f32);
                    scene_max[1] = scene_max[1].max(t.max[1] + oy as f32);
                    scene_max[2] = scene_max[2].max(t.max[2]);
                }
            }
        }
        if !scene_min[0].is_finite() { // nothing loaded — give F a sane box
            scene_min = [-1000.0; 3];
            scene_max = [1000.0; 3];
        }
    ...
    /// moves the target, and pan/zoom within the budget just changes the view matrix (state.rs
    /// builds it with `view_proj_anchored` against the anchor this returns).
    ...
    /// T(-origin) × M only changes the TRANSLATION column of a column-major matrix — subtract
    /// 3 numbers in f64 (precision kept) instead of a full 4×4 f64 multiply per object.
    ...
    pub fn clear(&mut self, color: wgpu::Color, view_proj: &Xform) -> anyhow::Result<()> {
        ...
        // Instance rebasing happens in rebase_anchor() (state.rs, before view_proj is built) —
        // pan/zoom only lands here as a new view matrix, ONE 64-byte uniform write.
...

        {
            ...
            //Edges - ONE draw for the WHOLE scene linework, in either style (LINEWORK_SOLID):
            // SOLID = instanced unit-cylinder template; FLAT = buffer-less camera-facing ribbons
            ...
            // Dots — same switch: SOLID = instanced sphere template; FLAT = circle billboards
            if self.glyph_count > 0 {
                if LINEWORK_SOLID {
                    ...
                    pass.set_bind_group(2, &self.instance_bind_group, &[]);
                    pass.set_bind_group(3, &self.glyph_bind_group, &[]);
                    pass.draw(0..3 * self.glyph_count, 0..1); // 3 verts per dot, no template
                }
                draws += 1;
            }
...
// 6 sides, not 12: thickness is screen-constant 2-4px at EVERY zoom, so the cross-section's
// roundness is never visible — 6 halves the per-segment cost (48→24 tris) with identical pixels.
