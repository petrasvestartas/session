...
mod tests {
    ...
    fn slow_drags_step_through_the_tiers() {
        ...
        assert_eq!(perf.drag_tier(), 1, "three slow frames: 135 ms to the first tier");
        frames(&mut perf, 3, 45.0, true);
        ...
        assert_eq!(perf.drag_tier(), TOP_TIER, "the next drag starts at the learned tier");
        frames(&mut perf, 30, 25.0, true);
        assert_eq!(perf.drag_tier(), TOP_TIER, "between the thresholds the tier holds");
        ...
        assert_eq!(perf.drag_tier(), 1, "two 100 ms frames: 200 ms to the first tier");
    ...
    fn a_pause_in_a_fast_drag_keeps_full_quality() {
        ...
        frames(&mut perf, 1, 400.0, true);
        frames(&mut perf, 3, 16.7, true);
        assert_eq!(perf.drag_tier(), 0, "a pause after the first frame of a drag");
    }
    ...
    fn slow_interaction_needs_a_run_of_slow_top_tier_frames() {
        let mut perf = Performance::new();
        assert!(!frames(&mut perf, 100, 150.0, false), "idle gaps are not slow frames");
