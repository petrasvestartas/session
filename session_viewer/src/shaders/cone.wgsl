...
struct CylinderSegment {
    p0:          vec3<f32>,
    radius:      f32,       // cone base radius (0 = use CYLINDER_RADIUS)
    p1:          vec3<f32>,
    instance_id: u32,
...
fn vs_main(
    ...
    // taper: 1.0 at base (lp.z = -0.5), 0.0 at tip (lp.z = +0.5)
    let taper = 0.5 - lp.z;
    let scaled    = vec3<f32>(lp.x * r * taper, lp.y * r * taper, lp.z * len);
