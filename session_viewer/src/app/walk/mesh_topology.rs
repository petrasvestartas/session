...
pub struct MeshTopo {
    pub edges: Vec<(usize, usize, u32)>, // Unique edges as (low key, high key, packed pen colour), in first-seen order.
    pub edge_faces: Vec<[u32; 2]>, // Per edge: the two faces that meet there, in arrival order; u32::MAX = none. A lone face always sits in slot 0.
    pub opposed: Vec<bool>, // Per edge: the two faces walk it in OPPOSITE directions, which is what consistent winding means locally. False says the pair disagrees, so one of the two normals points into the solid and the facing test must negate it. A lone face is trivially true.
    pub normals: Vec<Option<[f64; 3]>>, // Per face slot, in sorted-face-key order; None for a degenerate face.
    pub closed: bool, // Every edge has two faces: no border. Deliberately NOT "walked in both directions" - that reading called a badly wound solid open and threw the facing cull away wholesale.
}
