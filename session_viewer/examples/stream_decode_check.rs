...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("usage: stream_decode_check <file.pb>");
    let bytes = std::fs::read(&path).expect("read");
    let session = Session::pb_loads(&bytes).expect("parse");
    let g = session.order()[0].clone();
    let Some(Geometry::PointCloud(pc)) = session.lookup.get(&g) else {
        panic!("not a cloud")
    };
    ...
    assert!(
        worst <= tol,
        "coords differ by more than an f32 cast: {worst:e} > {tol:e}"
    );
    println!(
        "coords: {} values identical to the kernel (worst {worst:.3e}, f32 bound {tol:.3e})",
        streamed.len()
    );
    ...
    assert_eq!(
        (tag >> 3, tag & 7),
        (4, 2),
        "expected the colours field next"
    );
    ...
    let cover = (0..3)
        .map(|a| (phi[a] - plo[a]) / (fhi[a] - flo[a]))
        .fold(f64::INFINITY, f64::min);
    assert!(
        cover > 0.99,
        "the 2 M prefix box is only {cover:.3} of the cloud's - `fit` would be wrong"
    );
