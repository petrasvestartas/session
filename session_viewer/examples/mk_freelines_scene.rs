...
fn main() {
    let out = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "target/freelines.pb".into());
    let m = session_rust::Mesh::create_box(400.0, 400.0, 400.0);
    let edges = m.edges();
    let mut lines = Vec::new();
    for (a, b) in &edges {
        lines.push({
            let p0 = m.vertex_point(*a).unwrap();
            let p1 = m.vertex_point(*b).unwrap();
            session_rust::Line::new(p0[0], p0[1], p0[2], p1[0], p1[1], p1[2])
        });
    }
