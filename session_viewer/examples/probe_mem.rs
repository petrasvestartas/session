...
use session_rust::{Color, Geometry, Line, Mesh, NurbsCurve, Point, Polyline, Session};
...
fn live() -> f64 {
    LIVE.load(Relaxed) as f64 / 1.048576e6
}
fn mb(b: usize) -> f64 {
    b as f64 / 1.048576e6
}
...
fn main() {
    ...
    println!(
        "  file {file_mb:.1} MB | pb_loads {decode:?} | live heap {:.1} MB ({:.1}x file) | peak {:.1} MB | {:.2} M allocations",
        total,
        total / file_mb,
    ...
    println!(
        "  sizeof: Line {} Point {} Polyline {} Mesh {} Color {} NurbsCurve {} Geometry {}",
        std::mem::size_of::<Line>(),
        std::mem::size_of::<Point>(),
        std::mem::size_of::<Polyline>(),
        std::mem::size_of::<Mesh>(),
        std::mem::size_of::<Color>(),
        std::mem::size_of::<NurbsCurve>(),
        std::mem::size_of::<Geometry>()
    );
    let o = &s.objects;
    println!(
        "  counts: {} lines {} plines {} points {} meshes {} nurbs | lookup {} | graph v {} e {}",
        o.lines.len(),
        o.polylines.len(),
        o.points.len(),
        o.meshes.len(),
    ...
    let (mv, mf): (usize, usize) = o
        .meshes
        .iter()
        .fold((0, 0), |(a, b), m| (a + m.vertex.len(), b + m.face.len()));
    ...
    s.lookup = Default::default();
    step("lookup (guid -> Rc)", live(), &mut prev);
    s.graph = Default::default();
    step("graph", live(), &mut prev);
    s.tree = Default::default();
    step("tree", live(), &mut prev);
    {
        let lines = std::mem::take(&mut s.objects.lines);
        let mut owned: Vec<Line> = lines
            .into_iter()
            .filter_map(|l| std::rc::Rc::try_unwrap(l).ok())
            .collect();
        println!("  unwrapped {} lines", owned.len());
        let mut p2 = live();
        for l in owned.iter_mut() {
            l.name = String::new();
        }
        ...
        for l in owned.iter_mut() {
            l.linecolor.name = String::new();
        }
        step("  line.linecolor.name", live(), &mut p2);
        for l in owned.iter_mut() {
            l.linecolor = Color::new(0.0, 0.0, 0.0, 1.0);
        }
    ...
    s.objects.nurbscurves = Vec::new();
    step("nurbscurves", live(), &mut prev);
    s.objects.polylines = Vec::new();
    step("polylines", live(), &mut prev);
    s.objects.points = Vec::new();
    step("points", live(), &mut prev);
    {
        println!(
            "  sizeof VertexData {} | HashMap {} | Vec<usize> {}",
        ...
        let mut owned: Vec<Mesh> = meshes
            .into_iter()
            .filter_map(|m| std::rc::Rc::try_unwrap(m).ok())
            .collect();
        println!("  unwrapped {} meshes", owned.len());
        let mut p2 = live();
        for m in owned.iter_mut() {
            m.vertex = Default::default();
        }
        step("  mesh.vertex", live(), &mut p2);
        for m in owned.iter_mut() {
            m.face = Default::default();
        }
        step("  mesh.face", live(), &mut p2);
        for m in owned.iter_mut() {
            m.triangulation = Default::default();
        }
        step("  mesh.triangulation", live(), &mut p2);
        for m in owned.iter_mut() {
            m.facedata = Default::default();
            m.edgedata = Default::default();
            m.face_holes = Default::default();
        }
    ...
    s.objects.pointclouds = Vec::new();
    step("pointclouds", live(), &mut prev);
    drop(s);
    step("the rest", live(), &mut prev);
