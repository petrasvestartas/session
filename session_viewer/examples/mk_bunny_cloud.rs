...
fn main() {
    ...
    let src = a
        .first()
        .cloned()
        .unwrap_or_else(|| "assets/pb/mesh_bunny_grey.pb".into());
    let out = a
        .get(1)
        .cloned()
        .unwrap_or_else(|| "assets/pb/bunny_cloud.pb".into());
    ...
    let mesh = session
        .order()
        .into_iter()
        .find_map(|g| match session.lookup.get(&g) {
    ...
    let tri = |i: usize| {
        let f = [
            rm.indices[i * 3] as usize,
            rm.indices[i * 3 + 1] as usize,
            rm.indices[i * 3 + 2] as usize,
        ];
    ...
    for i in 0..ntri {
        let [a, b, c] = tri(i);
        let u = [
            b.position[0] as f64 - a.position[0] as f64,
            b.position[1] as f64 - a.position[1] as f64,
            b.position[2] as f64 - a.position[2] as f64,
        ];
        let v = [
            c.position[0] as f64 - a.position[0] as f64,
            c.position[1] as f64 - a.position[1] as f64,
            c.position[2] as f64 - a.position[2] as f64,
        ];
        let x = [
            u[1] * v[2] - u[2] * v[1],
            u[2] * v[0] - u[0] * v[2],
    ...
    for _ in 0..count {
        ...
        for k in 0..3 {
            coords.push(
                b0 * va.position[k] as f64
                    + b1 * vb.position[k] as f64
                    + b2 * vc.position[k] as f64,
            );
            normals.push(
                b0 * va.normal[k] as f64 + b1 * vb.normal[k] as f64 + b2 * vc.normal[k] as f64,
            );
        }
