...
check gate tests/depth/_gate.sh
check probe_matrix python3 tests/depth/_probe_matrix.py "$B/selftest" "$B/mk_hidden_line_probe" "$OUT/matrix"
...
for d in 1 4; do
    ...
    check "cylinder_hidden_$d" bash -c "set -- \$(python3 tests/depth/_count_colors.py '$OUT/cyl_hidden_$d.ppm'); [ \"\$4\" -eq 0 ]"
done
...
if [ -f "$FLOOR" ]; then
    check floor_census python3 tests/depth/_hidden_line_matrix.py "$B/selftest" "$B/census_plates" "$FLOOR" "$OUT/floor" --require-zero
else
