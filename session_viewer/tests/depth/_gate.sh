...
#
#   tests/depth/_gate.sh            -> prints "gate OK" or the first failure
#
...
if ! VIEWER_W=1400 VIEWER_H=900 "$B" "$out/local.ppm" assets/view_local.yaml >"$out/local.log" 2>&1; then
    cat "$out/local.log"
    echo "gate FAIL: local render"
    exit 1
fi
ink=$(grep -o "non-background pixels: [0-9]*" "$out/local.log" | grep -o "[0-9]*$")
...
probe() {
    ...
    read -r _ blue _ magenta < <(python3 tests/depth/_count_colors.py "$out/$name$z.ppm")
