...
(async () => {
  ...
  try {
    ...
    if (!process.env.AMBIENT_SPIN) {
      assert.deepEqual(on.mvp, off.mvp, 'SSAO preserves the camera projection and framing');
      assert.deepEqual(on.origin, off.origin, 'SSAO preserves the camera anchor');
    }
