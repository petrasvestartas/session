...
def calculate_morton_code(x: float, y: float, z: float, world_size: float = 100.0) -> int:
...
class BVH:
    ...
    def _merge_aabb(self, aabb1: BoundingBox, aabb2: BoundingBox) -> BoundingBox:
        ...
        min_x = min(
            aabb1.center.x - aabb1.half_size.x,
            aabb2.center.x - aabb2.half_size.x
        )
        min_y = min(
            aabb1.center.y - aabb1.half_size.y,
            aabb2.center.y - aabb2.half_size.y
        )
        min_z = min(
            aabb1.center.z - aabb1.half_size.z,
            aabb2.center.z - aabb2.half_size.z
        )
    ...
    def _find_collisions_recursive(
        ...
        if node.left:
            self._find_collisions_recursive(
                object_id, query_bbox, node.left,
                bounding_boxes, collisions, check_count
            )
        if node.right:
            self._find_collisions_recursive(
                object_id, query_bbox, node.right,
                bounding_boxes, collisions, check_count
            )
    ...
    def _aabb_intersect(self, aabb1: BoundingBox, aabb2: BoundingBox) -> bool:
        ...
        return (
            min1_x <= max2_x and max1_x >= min2_x and
    ...
    def check_all_collisions(
        ...
        -------
        tuple
            (collisions, check_count) where collisions is a list of (id1, id2)
            tuples representing colliding pairs, and check_count is the total
