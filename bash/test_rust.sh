...
# The PDF importer is a Rust-only extra behind --features pdf. On by default locally so the
# minitest binary keeps compiling it. In CI it is driven by SESSION_PDF, which the
...
# workflow). Its test is a `cargo test --features pdf` unit test in pdf_test.rs, not an Io test.
PDF_FEATURE=()
