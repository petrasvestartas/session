...
nlohmann::ordered_json Tree::to_json_data() const {
  ...
  return nlohmann::ordered_json{
      ...
      {"name", name},
      {"root",
       _root ? _root->to_json_data() : nlohmann::ordered_json(nullptr)}};
}

Tree Tree::from_json_data(const nlohmann::json &data) {
  auto tree = std::make_shared<Tree>(data["name"]);
  tree->guid = data["guid"];
  if (!data["root"].is_null()) {
    auto root = TreeNode::from_json_data(data["root"]);
    tree->add(root);
  }
  return *tree;
}
...
void Tree::to_json(const std::string &filepath) const {
  std::ofstream file(filepath);
  file << to_json_data().dump(4);
}

/// Deserialize from JSON file
Tree Tree::from_json(const std::string &filepath) {
